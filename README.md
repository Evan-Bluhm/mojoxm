...
- **Benchmark harness** (`benchmarks/`, run via `make bench-all`):
  37 analytic-solution gates tying schemes to closed-form reference
  ...
  3D (NP=35, NP=56).
  * **2D smooth (16):** `bench_advection_translation_2d` (rate >= 2.0)
...
- **2D kernel fusion (task #36 phase 2):** Each 2D physics path now
  ...
  * `maxwell_vol_lift_combine_rk_kernel_2d`       (NC=6, EM)
  ...
  per stage **3 -> 2 (-33%%)**.  All 37 analytic-solution gates remain
