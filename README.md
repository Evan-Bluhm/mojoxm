...
- **Benchmark harness** (`benchmarks/`, run via `make bench-all`):
  67 analytic-solution gates tying schemes to closed-form reference
  ...
  3D (NP=35, NP=56).
  * **2D smooth (23):** `bench_advection_translation_2d` (rate >= 2.0)
  ...
  * **2D shocks + EM (10):** `bench_euler_sod_2d`,
    ...
    `bench_maxwell_uniform_j_2d` + `_m` (uniform J / M on a periodic
    box with q=0 IC: dEx/dt = -c^2*Jx and dBz/dt = -Mz give exact
    linear ramps.  Closes a 2D feature-parity gap -- 3D Maxwell had
    `Maxwell.source_term` coupling J / M into the RK rhs, but the
    2D vol+lift kernel had no source-term plumbing at all.  Now
    both paths exercise the same physics).
...
- **2D kernel fusion (task #36 phase 2):** Each 2D physics path now
  ...
  * `maxwell_vol_lift_combine_rk_kernel_2d`       (NC=6, EM)
  ...
  per stage **3 -> 2 (-33%%)**.  All 67 analytic-solution gates remain
