...
- **Benchmark harness** (`benchmarks/`, run via `make bench-all`):
  50 analytic-solution gates tying schemes to closed-form reference
  ...
  3D (NP=35, NP=56).
  * **2D smooth (18):** `bench_advection_translation_2d` (rate >= 2.0)
  ...
  * **3D smooth (23):** `bench_advection_3d` + `_p3` (rate ~3.7) +
...
- **2D kernel fusion (task #36 phase 2):** Each 2D physics path now
  ...
  * `maxwell_vol_lift_combine_rk_kernel_2d`       (NC=6, EM)
  ...
  per stage **3 -> 2 (-33%%)**.  All 50 analytic-solution gates remain
