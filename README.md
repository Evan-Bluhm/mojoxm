...
| `src/vtu.mojo`                      |   606 | Zero-copy binary-appended VTU writer (one scalar field per frame)                      |
| `src/async_writer.mojo`             |   182 | pthread-based `writev()` scatter-gather file writer                                    |
| `src/frame_writer.mojo`             |   301 | Per-rank frame output: `FrameWriter[PhysT, P]`, auto rank-subdir + `mkdir -p` at init  |
...
| file                                          | lines | role                                                                            |
|-----------------------------------------------|-------|---------------------------------------------------------------------------------|
| `src/reference_2d.mojo`                       |   329 | 2D reference triangle: equispaced Lagrange, `D_ref`, `Lift_ref`, edge node maps |
| `src/reference_2d_gpu.mojo`                   |    80 | Float32 device mirror of `ReferenceElement2D` + `device_bytes()` helper         |
| `src/local_mesh_2d.mojo`                      |   435 | Periodic Kuhn-2-tri (per cube halved on diagonal) mesh + BC overlay             |
...
| `src/vtu_2d.mojo`                             |   482 | 2D VTU frame writer (incl. `dump_vtu_2d_frame_multi`) + `dump_pvd_collection` + `vtu_frame_name` helpers |
