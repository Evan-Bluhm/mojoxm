...
- **2D triangulated DG (GPU-only)**: `src/reference_2d.mojo` +
  ...
  **Tests:** GPU kernels are validated by self-consistent invariants
  rather than CPU reference code.  22 tests run under `make test-all`
...
- **Benchmark harness** (`benchmarks/`, run via `make bench-all`):
  74 analytic-solution gates tying schemes to closed-form reference
  ...
  * **2D shocks + EM (12):** `bench_euler_sod_2d`,
    ...
    NP=10).
  * **3D smooth (34):** `bench_advection_3d` + `_p3` (rate ~3.7) +
...
- **2D kernel fusion (phase 2):** Each 2D physics path now
  ...
  * `maxwell_vol_lift_combine_rk_kernel_2d`       (NC=6, EM)
  ...
  per stage **3 -> 2 (-33%%)**.  All 74 analytic-solution gates remain
