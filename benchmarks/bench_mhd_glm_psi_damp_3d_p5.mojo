# ======================================================================
# bench_mhd_glm_psi_damp_3d_p5 -- 3D GLM psi exponential decay at P=5
# ======================================================================
#
# P=5 (NP=56 nodes per tet) counterpart of bench_mhd_glm_psi_damp_3d_p3.
# Same uniform IC and same alpha_d>0 / c_h=0 setup, exact-exponential
# analytic decay psi(T) = A0 * exp(-alpha_d * T), but routed through
# Mesh[5] / Solver[IdealMHD, 5] / rk_stage_kernel[5].  Mesh shrunk to
# 2x2x2 since per-element work scales with NP^2.
#
# This is one of the few "linear decay rate" gates in the suite (per
# the standing-prompt "linear growth rate" theme).  Closes a P-parity
# gap on the GLM source-term path: the P=2/P=3 versions exist but
# higher-P variants didn't.  Validates that SSPRK3's operator-split
# damping multiplier `exp(-alpha_d * dt)` per step composes to the
# closed-form `exp(-alpha_d * T)` correctly at the highest 3D NP.
#
# Pass criteria (P=5, periodic 2x2x2, T=1, alpha_d=1, A0=0.1):
...
def damp_ic_kernel_p5(q: UnsafePointer[Float32, MutAnyOrigin], owned_elem_ids: UnsafePointer[Int32, MutAnyOrigin], elem_node_xyz: UnsafePointer[Float32, MutAnyOrigin], num_owned: Int):
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_mhd_glm_psi_damp_3d_p5: runs at np=1 only")
        return

    print("bench_mhd_glm_psi_damp_3d_p5 (3D GLM psi damping at P=5)")
    ...
    solver.ctx.enqueue_function[damp_ic_kernel_p5](
    ...
    for i in range(n_owned_nodes):
        for c in range(9):
            ...
            if isnan(v) or isinf(v):
                raise Error("bench_mhd_glm_psi_damp_3d_p5: non-finite output")
            if c == 8:
    ...
    if psi_rel > PSI_TOL_REL:
        raise Error("bench_mhd_glm_psi_damp_3d_p5 FAILED: psi rel err " + String(psi_rel) + " > " + String(PSI_TOL_REL))
    if max_state_drift > STATE_TOL:
        raise Error("bench_mhd_glm_psi_damp_3d_p5 FAILED: state drift " + String(max_state_drift) + " > " + String(STATE_TOL))

    print("=== bench_mhd_glm_psi_damp_3d_p5 PASSED ===")
    mpi.finalize()
