...
def main() raises:
    ...
    print("  P=", P, "  c_h=", C_H, "  T=", T_FINAL, "  amplitude=", AMPLITUDE)
    ...
    if err16 > L2_MAX_REL:
        raise Error(
            "bench_mhd_glm_psi_transport_2d FAILED: N=16 rel L2 " + String(err16) + " exceeds " + String(L2_MAX_REL)
        )
    if err24 > L2_MAX_REL:
        raise Error(
            "bench_mhd_glm_psi_transport_2d FAILED: N=24 rel L2 " + String(err24) + " exceeds " + String(L2_MAX_REL)
        )
    if err32 > L2_MAX_REL:
        raise Error(
            "bench_mhd_glm_psi_transport_2d FAILED: N=32 rel L2 " + String(err32) + " exceeds " + String(L2_MAX_REL)
        )
