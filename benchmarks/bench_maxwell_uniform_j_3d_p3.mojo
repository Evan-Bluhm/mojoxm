...
def main() raises:
    ...
    print("  P=", P, "  NP=", NP, "  mesh=", NX, "x", NY, "x", NZ, "   Jx=", JX, "   T=", T_FINAL)
    ...
    for i in range(n_owned_nodes):
        ...
        var values = [Float64(ey), Float64(ez), Float64(bx), Float64(by), Float64(bz)]
        for k in range(5):
            var v = values[k]
            if v < 0.0:
                v = -v
    ...
    print("  max |Ex - Ex_exact| / |Ex_exact| =", ex_rel, "  (threshold", EX_REL_TOL, ")")
    print("  max |Ey, Ez, Bx, By, Bz| =", max_other, "  (threshold", ZERO_COMPONENT_TOL, ")")

    if ex_rel > EX_REL_TOL:
        raise Error("bench_maxwell_uniform_j_3d_p3 FAILED: Ex deviation " + String(ex_rel) + " > " + String(EX_REL_TOL))
    if max_other > ZERO_COMPONENT_TOL:
        raise Error("bench_maxwell_uniform_j_3d_p3 FAILED: zero-component leak " + String(max_other) + " > " + String(ZERO_COMPONENT_TOL))
