# ======================================================================
# bench_maxwell_te_plane_wave_2d -- 2D TE plane wave on a periodic box
# ======================================================================
#
# TE-mode dual to bench_maxwell_plane_wave_2d.  Periodic [0, 1]^2 box,
# TE-mode plane wave traveling in +x at speed c:
#
#   Bz(x, y, t) =  cos(2 pi (x - c t))
#   Ey(x, y, t) = -c    cos(2 pi (x - c t))
#   Ex = Ez = Bx = By = 0  identically
#
# Closes a real coverage gap.  Both bench_maxwell_cavity_2d and
# bench_maxwell_plane_wave_2d are TM-mode (Ez != 0, Bx/By != 0), so
# they exercise the Fx2 = -c^2*By and Fx4 = -Ez paths in
# `maxwell_face_flux_kernel_2d` / the volume kernel.  TE mode (Bz/Ey
# nonzero, Ez/By zero) exercises the dual paths Fx1 = c^2*Bz and
# Fx5 = Ey.  A regression in those two terms or in their volume-RHS
# counterparts (Fy0 = -c^2*Bz, Fy4 = 0, Fy5 = -Ex etc.) would not have
# tripped any existing Maxwell gate.
#
# Pass criteria (P=2, NX=NY=16, single-rank, periodic):
#   * rel L2(state, all 6 components) < 1e-3
#   * leakage into the analytically-zero TE-zero components
#     (Ex, Ez, Bx, By) bounded < 5e-4 (same Kuhn-tet asymmetry floor
#     as the TM bench's zero-component check)
#   * no NaN / Inf
# ======================================================================
...
# Same thresholds as the TM bench: TE is the polarization dual and
# the kernel structure is symmetric, so dispersion and leakage floors
# should match.
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_maxwell_te_plane_wave_2d: runs at np=1 only")
        return

    print("bench_maxwell_te_plane_wave_2d (2D TE plane wave, periodic box)")
    ...
    # IC at t=0: Bz = cos(2 pi x), Ey = -c cos(2 pi x), rest 0.
    var two_pi = 2.0 * pi
    var c_d = Float64(C_LIGHT)
    ...
    for elem in range(gpu_mesh.num_elements):
        for nn in range(NP_p):
            for c in range(NC):
                ...
                sum_ic += ic * ic
                # In TE mode, Ex (0), Ez (2), Bx (3), By (4) are
    ...
    if rel > L2_MAX_REL:
        raise Error(String("bench_maxwell_te_plane_wave_2d FAILED: rel L2 ") + String(rel) + " > " + String(L2_MAX_REL))
    if max_zero_leak > ZERO_COMPONENT_MAX:
        raise Error(String("bench_maxwell_te_plane_wave_2d FAILED: zero-component ") + "leakage " + String(max_zero_leak) + " > " + String(ZERO_COMPONENT_MAX))
