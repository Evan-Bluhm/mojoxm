# ======================================================================
# bench_maxwell_cavity_2d_p3 -- 2D PEC TM standing wave at P=3 (NP=10)
# ======================================================================
#
# P=3 counterpart of bench_maxwell_cavity_2d.  Same TM-mode (1, 1)
# standing wave in a unit square cavity with PEC walls on all four
# sides:
#
...
#   omega = c * pi * sqrt(2)
#
# Period T = 2 pi / omega = sqrt(2) / c.  After one period the exact
# solution returns to the IC; rel L2(state - IC) is pure scheme
# dissipation + dispersion.
#
# Closes the 2D PEC-at-higher-P coverage gap (mirrors what
# bench_maxwell_cavity_3d_p3 closed for the 3D pipeline).  Every
# other 2D Maxwell P>=3 bench (plane_wave_2d_p3..p5, uniform_j/m_2d_p3)
# uses periodic BCs.  A regression in the 2D PEC reflection lift
# coefficient at P>=3 -- e.g. a stray sign on the odd-degree edge
# nodes that vanishes by symmetry at P=2 -- would not trip any
# existing 2D gate.
#
# Pass criteria (P=3, NX=NY=12, single-rank):
#   * rel L2(state, all 6 components) < 5e-4
#   * no NaN / Inf
# ======================================================================
...
# At P=3 the cavity mode is well-resolved at NX=NY=12 (same DOF count
# as P=2 NX=NY=16 -- 12*12*10 = 1440 vs 16*16*6 = 1536 vertices per
# component).  Measured rel L2 sits well below the P=2 floor of
# 2.8e-4; 5e-4 leaves headroom and catches PEC reflection / lift-
# operator bugs at P=3 without false-firing on Float32 wobble.
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_maxwell_cavity_2d_p3: runs at np=1 only")
        return

    print("bench_maxwell_cavity_2d_p3 (2D TM standing wave, PEC cavity, P=3)")
    ...
    for k_idx in range(n_q):
        ...
        if isnan(v) or isinf(v):
            raise Error("bench_maxwell_cavity_2d_p3: non-finite output")
    ...
    if rel > L2_MAX_REL:
        raise Error(String("bench_maxwell_cavity_2d_p3 FAILED: rel L2 ") + String(rel) + " > " + String(L2_MAX_REL))

    print("=== bench_maxwell_cavity_2d_p3 PASSED ===")
    mpi.finalize()
