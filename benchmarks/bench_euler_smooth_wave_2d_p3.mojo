# ======================================================================
# bench_euler_smooth_wave_2d_p3 -- 2D entropy-wave Euler at P=3
# ======================================================================
#
# P=3 counterpart of bench_euler_smooth_wave_2d.  Same entropy-wave
# IC (rho = rho0 + A sin(2 pi x) sin(2 pi y), uniform p, uniform u, v)
# on a periodic [0, 1]^2 mesh, but routed through LocalMesh2D[3] /
# euler_rk_stage_hllc_2d[3] with NP = 10 nodes per triangle.
#
# Pairs with bench_advection_translation_2d_p3 (P=3, single-comp) to
# cover NP=10 operator correctness across single + multi-component
# physics in 2D, mirroring the 3D coverage at P=3.
#
# Pass criteria (P=3, HLLC, sweep N=8, 12, 16):
#   * rel L2 at every N < 5e-4  (well-resolved at P=3)
#   * no NaN / Inf
#
# Note: at P=3 the 2D HLLC entropy-wave error sits at the Float32
# roundoff floor (~5e-5) across the entire sweep, so monotone
# refinement and convergence-rate checks don't apply -- the scheme
# is more accurate than Float32 representation.  bench_advection_
# translation_2d_p3 (single component, larger amplitude) is the
# rate-test for the 2D P=3 path; this benchmark is a multi-component
# ABSOLUTE-L2 regression sentinel only.
# ======================================================================
...
def _run(N: Int) raises -> Float64:
    ...
    for k in range(n_q):
        ...
        if isnan(v) or isinf(v):
            raise Error("bench_euler_smooth_wave_2d_p3: non-finite output")
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_euler_smooth_wave_2d_p3: runs at np=1 only")
        return
    ...
    if err8 > L2_MAX_REL_AT_16:
        raise Error(
            ...
            + String(err8) + " exceeds " + String(L2_MAX_REL_AT_16)
        )
    if err12 > L2_MAX_REL_AT_16:
        raise Error(
            "bench_euler_smooth_wave_2d_p3 FAILED: rel L2 at N=12 "
            + String(err12) + " exceeds " + String(L2_MAX_REL_AT_16)
        )
