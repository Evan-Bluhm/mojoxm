# ======================================================================
# bench_shallow_water_wave_3d_p4 -- 3D linear SW wave at P=4 (NP=35)
# ======================================================================
#
# P=4 counterpart of bench_shallow_water_wave_3d_p3.  Same small-
# amplitude linear shallow-water wave on a 3D periodic mesh
# (F^z = 0, NZ=2 stacked sheet) but routed through Mesh[4] /
# Solver[ShallowWater, 4] with NP=35 nodes per tet.
#
# Pairs with bench_advection_3d_p4 + bench_euler_smooth_wave_3d_p4
# + bench_maxwell_plane_wave_3d_p4 to bring 3D SW (NC=3) up to NP=35
# coverage parity for smooth physics.
#
# Pass criteria (P=4, periodic, single-rank, sweep N=6, 8, 10):
#   * rel L2 < 5e-4 at every N (Float32 floor + A/H=0.01 nonlinear
#     correction floor; same plateau as P=3)
...
def _run(N: Int) raises -> RunResult:
    ...
    for k in range(n_owned_dof):
        ...
        if isnan(v) or isinf(v):
            raise Error("bench_shallow_water_wave_3d_p4: non-finite output")
...
def main() raises:
    ...
    mpi.init()
    print("bench_shallow_water_wave_3d_p4 (linear SW wave, P=4)")
    ...
    if not all_ok:
        raise Error("bench_shallow_water_wave_3d_p4 FAILED")
    print("=== bench_shallow_water_wave_3d_p4 PASSED ===")
    mpi.finalize()
