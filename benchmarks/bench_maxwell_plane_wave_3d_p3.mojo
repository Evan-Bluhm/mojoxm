# ======================================================================
# bench_maxwell_plane_wave_3d_p3 -- 3D TM plane wave at P=3 (NP=20)
# ======================================================================
#
# 3D analog of bench_maxwell_plane_wave_2d_p3.  Same TM-mode plane
# wave traveling in +x at speed c on a triply-periodic [0,1]^3 box,
# but routed through Mesh[3] / Solver[Maxwell, 3] with NP = 20
# nodes per tet.
#
# Pairs with bench_advection_3d_p3, bench_euler_smooth_wave_3d_p3,
# bench_shallow_water_wave_3d_p3, and bench_mhd_alfven_3d_p3 to
# bring 3D Maxwell to NP=20 coverage parity with the other 3D
# smooth physics.
#
# Pass criteria (P=3, NX=12 NY=NZ=4, single-rank):
#   * rel L2(state, all 6 components) < 1e-3 (well below the
#     P=2 floor; P=3 dispersion is small enough that the Kuhn-tet
#     asymmetry under Rusanov dominates)
#   * leakage into the analytically-zero TM-zero components
#     (Ex, Ey, Bx, Bz) bounded < 5e-4
#   * no NaN / Inf
# ======================================================================
...
# At P=3 the dispersion error drops well below the Kuhn-tet
# asymmetry floor.  Loose 1e-3 catches operator regressions; tighter
# would risk false positives on Float32 wobble across hardware.
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_maxwell_plane_wave_3d_p3: runs at np=1 only")
        return
    ...
    var solver = Solver[Maxwell, P](ctx^, mesh^, halo^, physics^, D_ref^, Lift_ref^, node_weights^)
    ...
    for k in range(n_owned_dof):
        ...
        if isnan(v_now) or isinf(v_now):
            raise Error("bench_maxwell_plane_wave_3d_p3: non-finite output")
    ...
    if max_zero_leak > ZERO_COMPONENT_MAX:
        raise Error(String("bench_maxwell_plane_wave_3d_p3 FAILED: zero-component ") + "leakage " + String(max_zero_leak) + " > " + String(ZERO_COMPONENT_MAX))

    print("=== bench_maxwell_plane_wave_3d_p3 PASSED ===")
    mpi.finalize()
