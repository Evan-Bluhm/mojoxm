...
def main() raises:
    ...
    if err8 > L2_MAX_REL:
        raise Error(
            "bench_mhd_glm_psi_transport_2d_p5 FAILED: NX=8 rel L2 " + String(err8) + " exceeds " + String(L2_MAX_REL)
        )
    if err12 > L2_MAX_REL:
        raise Error(
            "bench_mhd_glm_psi_transport_2d_p5 FAILED: NX=12 rel L2 " + String(err12) + " exceeds " + String(L2_MAX_REL)
        )
    if err16 > L2_MAX_REL:
        raise Error(
            "bench_mhd_glm_psi_transport_2d_p5 FAILED: NX=16 rel L2 " + String(err16) + " exceeds " + String(L2_MAX_REL)
        )
