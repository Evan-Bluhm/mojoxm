# ======================================================================
# bench_mhd_glm_psi_transport_2d_p5 -- GLM psi/Bx wave at P=5 (NP=21)
# ======================================================================
#
# P=5 counterpart of bench_mhd_glm_psi_transport_2d_p3.  Same linear-
# wave (psi, Bx) coupling test on a periodic [0,1]^2 box, routed
# through LocalMesh2D[5] / mhd_glm_rk_stage_2d[5] with NP=21 nodes
# per triangle.  Mesh sweep tightened from 12/16/24 (P=3) to 8/12/16
# since per-element work scales with NP^2 and dt tightens to
# 1/(2P+1)=1/11.
#
# Pairs with bench_mhd_glm_psi_damp_2d_p5 (the operator-split decay
# rate gate) for full 2D MHD GLM P=5 coverage:
#   transport (c_h>0, alpha_d=0)  -- this bench
#   damp      (c_h=0, alpha_d>0)  -- bench_mhd_glm_psi_damp_2d_p5
#
# Pass criteria (P=5, sweep N = 8, 12, 16, c_h=1):
...
def _run(NX: Int) raises -> Float64:
    ...
    for k in range(n_q):
        ...
        if isnan(v) or isinf(v):
            raise Error("bench_mhd_glm_psi_transport_2d_p5: non-finite")
    ...
    if psi_max > amp_bound:
        raise Error(String("bench_mhd_glm_psi_transport_2d_p5 FAILED: psi_max ") + String(psi_max) + " exceeds 1.1 * AMPLITUDE " + String(amp_bound))
...
def main() raises:
    ...
    print("  P=", P, "  NP=", num_tri_nodes_2d(P), "  c_h=", C_H, "  T=", T_FINAL, "  amplitude=", AMPLITUDE)
    ...
    if err8 > L2_MAX_REL:
        raise Error("bench_mhd_glm_psi_transport_2d_p5 FAILED: NX=8 rel L2 " + String(err8) + " exceeds " + String(L2_MAX_REL))
    if err12 > L2_MAX_REL:
        raise Error("bench_mhd_glm_psi_transport_2d_p5 FAILED: NX=12 rel L2 " + String(err12) + " exceeds " + String(L2_MAX_REL))
    if err16 > L2_MAX_REL:
        raise Error("bench_mhd_glm_psi_transport_2d_p5 FAILED: NX=16 rel L2 " + String(err16) + " exceeds " + String(L2_MAX_REL))
