# ======================================================================
# bench_euler_hydrostatic_2d -- 2D Euler gravity source-term gate
# ======================================================================
#
# 2D analog of bench_euler_hydrostatic_3d.  Closes a 2D feature-parity
# gap parallel to the 2D Maxwell J / M source landed in the previous
# commits: 3D Euler had `Euler.source_term` coupling
# (gx, gy, gz) into the RK rhs (d(rho u_i)/dt += rho * g_i,
# dE/dt += rho * (u . g)) and bench_euler_hydrostatic_3d gated it.
# The 2D Euler GPU vol+lift kernel had no source-term plumbing at
# all -- the shared `examples/euler_rising_bubble` sat in 3D only.
# This commit adds gx / gy threading to the 2D Euler kernels (both
# Rusanov and HLLC paths) and a hydrostatic gate that pairs with
# the 3D bench.
#
# Cleanest analytic test: hydrostatic balance.  Constant-density
# fluid at rest with gravity g = (0, gy) (gy < 0):
#
#   rho(y) = rho0
#   u = v = 0
#   p(y)   = p0 + rho0 * gy * y
#
# Hydrostatic balance: -dp/dy = -rho0 * gy = rho(y) * |gy|, so the
...
#
# BC_INTERIOR (periodic) in x; BC_WALL on +/-y (slip wall reflects
# v=0 trivially); periodic in y would conflict with the
# stratification (p(L) != p(0)).
#
# Pass criteria (P=2, NX=4, NY=16, T=1):
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_euler_hydrostatic_2d: runs at np=1 only")
        return

    print("bench_euler_hydrostatic_2d (2D hydrostatic balance, gravity gate)")
    ...
    for i in range(n_nodes):
        ...
        if isnan(rho_now) or isinf(rho_now):
            raise Error("bench_euler_hydrostatic_2d: non-finite output")
        var u = rhou / rho_now
        var v = rhov / rho_now
    ...
    if max_v > VMAX_TOL:
        raise Error(String("bench_euler_hydrostatic_2d FAILED: |v_mag| ") + String(max_v) + " > " + String(VMAX_TOL))
    if max_rho_dev > RHO_REL_TOL:
        raise Error(String("bench_euler_hydrostatic_2d FAILED: density drift ") + String(max_rho_dev) + " > " + String(RHO_REL_TOL))
    if max_p_dev > P_REL_TOL:
        raise Error(String("bench_euler_hydrostatic_2d FAILED: pressure drift ") + String(max_p_dev) + " > " + String(P_REL_TOL))

    print("=== bench_euler_hydrostatic_2d PASSED ===")
    mpi.finalize()
