# ======================================================================
# bench_mhd_glm_psi_transport_3d_p5 -- 3D GLM linear psi/Bx wave at P=5
# ======================================================================
#
# P=5 (NP=56) counterpart of bench_mhd_glm_psi_transport_3d_p3.  Same
# psi=A*sin(2pi x/Lx) IC on a rest state, same one-period (T=Lx/c_h)
# standing-wave horizon, but routed through Mesh[5] / Solver[IdealMHD,
# 5] / rk_stage_kernel[5] so the GLM transport coupling in
# `IdealMHD.internal_flux` is exercised at NP=56.  Mesh shrunk to
# 4x2x2 (vs 16x4x4 at P=3) since per-element work scales with NP^2
# and dt tightens to 1/(2P+1)=1/11.
#
# Pairs with bench_mhd_glm_psi_damp_3d_p5 (the other P=5 GLM rate
# gate) for full 3D MHD GLM P=5 coverage:
#   transport (c_h>0, alpha_d=0)  -- this bench
#   damp      (c_h=0, alpha_d>0)  -- bench_mhd_glm_psi_damp_3d_p5
#
# Pass criteria (P=5, NX=4, NY=NZ=2, c_h=1, T=1):
#   * rel L2(state) < 5e-4 (Float32 floor at NP=56)
...
def transport_ic_kernel_p5(q: UnsafePointer[Float32, MutAnyOrigin], owned_elem_ids: UnsafePointer[Int32, MutAnyOrigin], elem_node_xyz: UnsafePointer[Float32, MutAnyOrigin], num_owned: Int):
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_mhd_glm_psi_transport_3d_p5: runs at np=1 only")
        return

    print("bench_mhd_glm_psi_transport_3d_p5 (3D GLM linear psi/Bx wave at P=5)")
    ...
    solver.ctx.enqueue_function[transport_ic_kernel_p5](
    ...
    for k in range(n_owned_dof):
        ...
        if isnan(v) or isinf(v):
            raise Error("bench_mhd_glm_psi_transport_3d_p5: non-finite output")
    ...
    if psi_max > amp_bound:
        raise Error(String("bench_mhd_glm_psi_transport_3d_p5 FAILED: psi_max ") + String(psi_max) + " exceeds 1.1 * AMPLITUDE " + String(amp_bound))
    ...
    if rel_l2 > L2_MAX_REL:
        raise Error("bench_mhd_glm_psi_transport_3d_p5 FAILED: rel L2 " + String(rel_l2) + " > " + String(L2_MAX_REL))

    print("=== bench_mhd_glm_psi_transport_3d_p5 PASSED ===")
    mpi.finalize()
