...
def main() raises:
    ...
    if max_rho_dev > RHO_REL_TOL:
        raise Error(String("bench_euler_hydrostatic_2d_p3 FAILED: density drift ") + String(max_rho_dev) + " > " + String(RHO_REL_TOL))
    if max_p_dev > P_REL_TOL:
        raise Error(String("bench_euler_hydrostatic_2d_p3 FAILED: pressure drift ") + String(max_p_dev) + " > " + String(P_REL_TOL))
