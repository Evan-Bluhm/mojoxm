...
def main() raises:
    ...
    if max_h_dev > H_TOL:
        raise Error(
            "bench_shallow_water_inflow_2d_rusanov FAILED: max |h - H0| " + String(max_h_dev) + " > " + String(H_TOL)
        )
    if hu_rel > HU_REL_TOL:
        raise Error(
            "bench_shallow_water_inflow_2d_rusanov FAILED: hu rel err " + String(hu_rel) + " > " + String(HU_REL_TOL)
        )
    if max_hv > HV_TOL:
