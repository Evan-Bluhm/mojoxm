# ======================================================================
# bench_maxwell_uniform_j_2d_p3 -- 2D Maxwell J source at P=3 (NP=10)
# ======================================================================
#
# P=3 (NP=10) counterpart of bench_maxwell_uniform_j_2d.  Same
# uniform J source-term gate, but routed through LocalMesh2D[3] /
# maxwell_rk_stage_2d[3] so the 2D Maxwell J path landed in commit
# e76b90a is exercised at NP=10.  Locks in the higher-order
# behavior of the just-added source-term parity work; the kernel
# is templated on P, so P=3 should "just work" at the Float32 floor.
#
# Same analytic test: uniform J on a periodic box with q = 0 at t = 0
# means Ex(T) = -c^2 * Jx * T (exact), all other components zero.
#
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_maxwell_uniform_j_2d_p3: runs at np=1 only")
        return

    print("bench_maxwell_uniform_j_2d_p3 (uniform J source-term gate)")
    ...
    for i in range(n_nodes):
        ...
        if d_ex < 0.0: d_ex = -d_ex
        var rel = d_ex / abs(Ex_expect)
        if rel > max_ex_rel: max_ex_rel = rel
        ...
        if av_bz > max_zero: max_zero = av_bz

    print("  Ex(T) expected =", Ex_expect)
    print("  max |Ex - exact| / |Ex_exact| =", max_ex_rel,
          "  (threshold", EX_REL_TOL, ")")
    print("  max |Ey,Ez,Bx,By,Bz| =", max_zero,
          "  (threshold", ZERO_COMPONENT_TOL, ")")

    if max_ex_rel > EX_REL_TOL:
        raise Error(
            String("bench_maxwell_uniform_j_2d_p3 FAILED: Ex rel err ")
            + String(max_ex_rel) + " > " + String(EX_REL_TOL)
        )
    if max_zero > ZERO_COMPONENT_TOL:
        raise Error(
            String("bench_maxwell_uniform_j_2d_p3 FAILED: zero-component drift ")
