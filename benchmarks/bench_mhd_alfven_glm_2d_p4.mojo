...
def _run() raises -> Float64:
    ...
    if psi_max > PSI_TOL:
        raise Error(
            String("bench_mhd_alfven_glm_2d_p4 FAILED: psi_max ") + String(psi_max) + " > tol " + String(PSI_TOL)
        )
    return rel_l2
...
def main() raises:
    ...
    if rel_l2 > L2_MAX_REL:
        raise Error(
            "bench_mhd_alfven_glm_2d_p4 FAILED: rel L2 " + String(rel_l2) + " exceeds threshold " + String(L2_MAX_REL)
        )
