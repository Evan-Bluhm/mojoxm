# ======================================================================
# bench_mhd_alfven_3d_p5 -- 3D linear Alfven wave at P=5 (NP=56)
# ======================================================================
#
# P=5 counterpart of bench_mhd_alfven_3d_p4.  Same linearly-polarised
# right-going Alfven-wave IC in a thin 3D slab, but routed through
# Mesh[5] / Solver[IdealMHD, 5] / NP=56.  Highest-NC physics
# validated at NP=56 in 3D -- exercises the cooperative shared-
# memory rk_stage_kernel at NC=9 / NP=56 = 504 q values per element,
# the largest per-element working set in the suite.
#
# Pairs with the just-added P=5 / NP=56 gates for advection
# (NC=1), Euler (NC=5), Maxwell (NC=6), and SW (NC=3) to bring
# 3D coverage parity for smooth physics up to P=5 across all 5
# physics paths.  Closes the 3D MHD-GLM P-parity sweep
# (P=2/3/4/5).
#
# Pass criteria (P=5, NX=8, NY=NZ=4, single-rank, periodic):
#   * rel L2(9-component state) < 5e-3 (same A^2~0.01 nonlinear
#     correction floor as P=3/P=4 -- IC sets the floor, not the
#     scheme)
#   * no NaN / Inf
# ======================================================================
...
# Same nonlinear A^2~0.01 floor as P=2/P=3/P=4 (~3.6e-3).  5e-3 is
# the usual margin -- catches MHD operator regressions at NP=56
# without false-firing on the IC-set floor.
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_mhd_alfven_3d_p5: runs at np=1 only")
        return

    print("bench_mhd_alfven_3d_p5 (3D Alfven wave, P=5, one period)")
    ...
    for k in range(n_owned_dof):
        ...
        if isnan(v_now) or isinf(v_now):
            raise Error("bench_mhd_alfven_3d_p5: non-finite output at " + String(k))
    ...
    if rel_l2 > L2_MAX_REL:
        raise Error("bench_mhd_alfven_3d_p5 FAILED: rel L2 " + String(rel_l2) + " exceeds threshold " + String(L2_MAX_REL))
    print("=== bench_mhd_alfven_3d_p5 PASSED ===")
    mpi.finalize()
