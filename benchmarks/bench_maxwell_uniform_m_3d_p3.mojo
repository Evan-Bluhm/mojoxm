...
def main() raises:
    ...
    var physics = Maxwell(
        ...
        Float32(0.0), Float32(0.0), MZ,        # M = (0, 0, Mz)
    )
    ...
    for i in range(n_owned_nodes):
        ...
        if bz_dev < 0.0: bz_dev = -bz_dev
        if bz_dev > max_bz_dev: max_bz_dev = bz_dev
        var other_max = Float64(0.0)
        var values = [Float64(ex), Float64(ey), Float64(ez), Float64(bx),
                      Float64(by)]
        for k in range(5):
            var v = values[k]
            if v < 0.0: v = -v
