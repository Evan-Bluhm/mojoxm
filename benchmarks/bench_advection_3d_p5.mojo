# ======================================================================
# bench_advection_3d_p5 -- 3D Gaussian one-period advection at P=5
# ======================================================================
#
# P=5 counterpart of bench_advection_3d_p4.  Same Gaussian transport
# in a triply-periodic [0, 1]^3 box, routed through Mesh[5] +
# Solver[Advection, 5] with NP = 56 nodes per tet.  Expected
# asymptotic convergence rate is P+1 = 6.
#
# Highest-order analytic-rate gate in the 3D suite, paired with the
# 2D counterpart at P=5.
#
# Pass criteria (P=5, SSPRK3, CFL=0.1, single-rank, sweep N=6, 8, 12):
#   * rel L2 at N=12 < 5e-3
#   * monotone refinement
#   * rate >= 2.5 between at least one consecutive pair
#     (theoretical 6; observed ~5.33 / ~3.65 in current code; the
#     second pair drops because N=12 hits the Float32 floor at this
#     order)
# ======================================================================
...
def _run(N: Int) raises -> Float64:
    ...
    for k in range(n_owned_dof):
        ...
        if isnan(v_now) or isinf(v_now):
            raise Error("bench_advection_3d_p5: non-finite output")
...
def main() raises:
    ...
    if err12 > L2_MAX_REL_AT_12:
        raise Error("bench_advection_3d_p5 FAILED: rel L2 at N=12 " + String(err12) + " exceeds " + String(L2_MAX_REL_AT_12))
    if not (err6 > err8 and err8 > err12):
        raise Error("bench_advection_3d_p5 FAILED: rel L2 did not decrease " + "monotonically (6: " + String(err6) + ", 8: " + String(err8) + ", 12: " + String(err12) + ")")
    ...
    if rate_68 < RATE_MIN and rate_812 < RATE_MIN:
        raise Error("bench_advection_3d_p5 FAILED: observed rates " + String(rate_68) + " and " + String(rate_812) + " both below " + String(RATE_MIN))
