# ======================================================================
# bench_euler_smooth_wave_2d_p5 -- 2D entropy-wave Euler at P=5
# ======================================================================
#
# P=5 counterpart of bench_euler_smooth_wave_2d_p4.  Same entropy-wave
# IC (rho = rho0 + A sin(2 pi x) sin(2 pi y), uniform p, uniform u, v)
# on a periodic [0, 1]^2 mesh, but routed through LocalMesh2D[5] /
# euler_rk_stage_hllc_2d[5] with NP = 21 nodes per triangle.
#
# Highest-order multi-component Euler gate in the suite.  Mirrors the
# P=5 advection coverage (`bench_advection_translation_2d_p5`, NP=21,
# single-component); closes the multi-component-Euler P-parity gap up
# to P=5.  At NP=21 in 2D Euler the per-element register pressure and
# operator size hit comptime extremes that earlier benches don't reach,
# so this is also a structural smoke test for the high-P operator
# construction (Vandermonde inverse, Lift_ref, mass-matrix weights).
#
# Pass criteria (P=5, HLLC, periodic, sweep N=4, 6, 8):
#   * rel L2 at every N < 1.5e-4  (Float32-floor regime)
#   * no NaN / Inf
#
# At P=5 / NP=21 the entropy wave sits at the Float32 round-off floor
# across the entire sweep -- exactly the same plateau as P=3 / P=4
# since the scheme is more accurate than Float32 representation can
# resolve at this amplitude.  Absolute-L2 regression sentinel for the
# highest-P Euler kernels.
# ======================================================================
...
# at P=3 / P=4 since the scheme is more accurate than Float32 can
# represent at this amplitude).  1.5e-4 is ~3x the empirical floor.
...
def _run(N: Int) raises -> Float64:
    ...
    for k in range(n_q):
        ...
        if isnan(v) or isinf(v):
            raise Error("bench_euler_smooth_wave_2d_p5: non-finite output")
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_euler_smooth_wave_2d_p5: runs at np=1 only")
        return
    ...
    if err4 > L2_MAX_REL:
        raise Error("bench_euler_smooth_wave_2d_p5 FAILED: rel L2 at N=4 " + String(err4) + " exceeds " + String(L2_MAX_REL))
    if err6 > L2_MAX_REL:
        raise Error("bench_euler_smooth_wave_2d_p5 FAILED: rel L2 at N=6 " + String(err6) + " exceeds " + String(L2_MAX_REL))
    if err8 > L2_MAX_REL:
        raise Error("bench_euler_smooth_wave_2d_p5 FAILED: rel L2 at N=8 " + String(err8) + " exceeds " + String(L2_MAX_REL))
