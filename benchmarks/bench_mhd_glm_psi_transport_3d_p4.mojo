# ======================================================================
# bench_mhd_glm_psi_transport_3d_p4 -- 3D GLM linear psi/Bx wave at P=4
# ======================================================================
#
# P=4 (NP=35) intermediate between bench_mhd_glm_psi_transport_3d_p3
# (NP=20) and bench_mhd_glm_psi_transport_3d_p5 (NP=56).  Same psi=
# A*sin(2pi x/Lx) IC + standing-wave horizon.  Mesh 8x4x4 between
# P=3's 16x4x4 and P=5's 4x2x2, balancing per-element NP^2 work
# against dt tightening 1/(2P+1)=1/9.
#
# Closes the P=4 GLM transport rate-gate gap; pairs with
# bench_mhd_glm_psi_damp_3d_p3 + bench_mhd_alfven_3d_p4 for full
# 3D MHD GLM P=4 coverage of the three rate paths.
#
# Pass criteria (P=4, NX=8, NY=NZ=4, c_h=1, T=1):
#   * rel L2(state) < 5e-4 (Float32 floor at NP=35)
...
def transport_ic_kernel_p4(q: UnsafePointer[Float32, MutAnyOrigin], owned_elem_ids: UnsafePointer[Int32, MutAnyOrigin], elem_node_xyz: UnsafePointer[Float32, MutAnyOrigin], num_owned: Int):
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_mhd_glm_psi_transport_3d_p4: runs at np=1 only")
        return

    print("bench_mhd_glm_psi_transport_3d_p4 (3D GLM linear psi/Bx wave at P=4)")
    ...
    solver.ctx.enqueue_function[transport_ic_kernel_p4](
    ...
    for k in range(n_owned_dof):
        ...
        if isnan(v) or isinf(v):
            raise Error("bench_mhd_glm_psi_transport_3d_p4: non-finite output")
    ...
    if psi_max > amp_bound:
        raise Error(String("bench_mhd_glm_psi_transport_3d_p4 FAILED: psi_max ") + String(psi_max) + " exceeds 1.1 * AMPLITUDE " + String(amp_bound))
    ...
    if rel_l2 > L2_MAX_REL:
        raise Error("bench_mhd_glm_psi_transport_3d_p4 FAILED: rel L2 " + String(rel_l2) + " > " + String(L2_MAX_REL))

    print("=== bench_mhd_glm_psi_transport_3d_p4 PASSED ===")
    mpi.finalize()
