# ======================================================================
# bench_euler_hydrostatic_2d_p5 -- 2D Euler gravity at P=5 (NP=21)
# ======================================================================
#
# P=5 (NP=21) counterpart of bench_euler_hydrostatic_2d_p3.  Same
# stratified rest IC, same gravity gy<0, but routed through
# LocalMesh2D[5] / euler_rk_stage_hllc_2d[5] so the 2D Euler
# gravity source path is exercised at NP=21 -- the highest 2D Euler
# NP the bench harness gates end-to-end.  Mesh size unchanged
# (4x12); per-element volume work scales with NP^2 and dt tightens
# to 1/(2P+1)=1/11, but the suite still runs in well under a second.
#
# Pass criteria (P=5, NX=4, NY=12, T=1):
...
def main() raises:
    ...
    if max_v > VMAX_TOL:
        raise Error(String("bench_euler_hydrostatic_2d_p5 FAILED: |v_mag| ") + String(max_v) + " > " + String(VMAX_TOL))
    if max_rho_dev > RHO_REL_TOL:
        raise Error(String("bench_euler_hydrostatic_2d_p5 FAILED: density drift ") + String(max_rho_dev) + " > " + String(RHO_REL_TOL))
    if max_p_dev > P_REL_TOL:
        raise Error(String("bench_euler_hydrostatic_2d_p5 FAILED: pressure drift ") + String(max_p_dev) + " > " + String(P_REL_TOL))
