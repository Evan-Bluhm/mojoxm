...
def main() raises:
    ...
    print("  P= 2   mesh=", NX, "x", NY, "x", NZ,
          "   M=", U0 / sqrt(GAMMA * P0 / RHO0), "   T=", T_FINAL)
    ...
    var bcs = BoundaryConditions(
        ...
        BC_INTERIOR, BC_INTERIOR,      # -z, +z (periodic)
    )
    var mesh = Mesh(
    ...
    var E0 = (
        P0 / (GAMMA - Float32(1.0))
        + Float32(0.5) * RHO0 * (U0 * U0)
    )
    var physics = Euler(
        GAMMA, MIN_DENSITY, MIN_PRESSURE, FLUX_HLLEC, False,
        Float32(0.0), Float32(0.0), Float32(0.0),    # no gravity
        rho0_inflow, rhou0_inflow,
        rhov0_inflow, rhow0_inflow, E0,
    )
    ...
    for i in range(n_owned_nodes):
        ...
        if (isnan(rho_now) or isinf(rho_now) or isnan(rhou_now)
            or isinf(rhou_now) or isnan(rhov_now) or isinf(rhov_now)
            or isnan(rhow_now) or isinf(rhow_now) or isnan(E_now)
            or isinf(E_now)):
            raise Error("bench_euler_inflow_3d: non-finite output")
        var d_rho = Float64(rho_now - RHO0)
        if d_rho < 0.0: d_rho = -d_rho
        if d_rho > max_rho_dev: max_rho_dev = d_rho
        var d_rhou = Float64(rhou_now - RHO0 * U0)
        if d_rhou < 0.0: d_rhou = -d_rhou
        if d_rhou > max_rhou_dev: max_rhou_dev = d_rhou
        var a_rhov = Float64(rhov_now)
        if a_rhov < 0.0: a_rhov = -a_rhov
        if a_rhov > max_rhovw: max_rhovw = a_rhov
        var a_rhow = Float64(rhow_now)
        if a_rhow < 0.0: a_rhow = -a_rhow
        if a_rhow > max_rhovw: max_rhovw = a_rhow
        var d_E = Float64(E_now - E0)
        if d_E < 0.0: d_E = -d_E
        if d_E > max_E_dev: max_E_dev = d_E
    ...
    if max_rhovw > RHOVW_TOL:
        raise Error(
            ...
            + String(max_rhovw) + " > " + String(RHOVW_TOL)
        )
    if E_rel > E_REL_TOL:
        raise Error(
