...
def main() raises:
    ...
    if rho_max > RHO_L + BOUNDS_SLACK:
        raise Error(
            String("bench_euler_sod_3d_p4 FAILED: rho_max ") + String(rho_max) + " overshot RHO_L=" + String(RHO_L)
        )
    ...
    if rho_min < RHO_R - BOUNDS_SLACK:
        raise Error(
            String("bench_euler_sod_3d_p4 FAILED: rho_min ") + String(rho_min) + " undershot RHO_R=" + String(RHO_R)
        )
