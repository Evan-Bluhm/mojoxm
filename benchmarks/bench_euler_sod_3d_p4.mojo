...
def sod_ic_kernel_p4(q: UnsafePointer[Float32, MutAnyOrigin], owned_elem_ids: UnsafePointer[Int32, MutAnyOrigin], elem_node_xyz: UnsafePointer[Float32, MutAnyOrigin], num_owned: Int):
...
def main() raises:
    ...
    solver.ctx.enqueue_function[sod_ic_kernel_p4](
    ...
    if rho_max > RHO_L + BOUNDS_SLACK:
        raise Error(String("bench_euler_sod_3d_p4 FAILED: rho_max ") + String(rho_max) + " overshot RHO_L=" + String(RHO_L))
    if rho_min < Float32(0.0):
        raise Error(String("bench_euler_sod_3d_p4 FAILED: rho_min ") + String(rho_min) + " negative (positivity lost)")
    if rho_min < RHO_R - BOUNDS_SLACK:
        raise Error(String("bench_euler_sod_3d_p4 FAILED: rho_min ") + String(rho_min) + " undershot RHO_R=" + String(RHO_R))
    ...
    if rel > MASS_TOL_REL:
        raise Error(String("bench_euler_sod_3d_p4 FAILED: mass drift ") + String(rel * 100.0) + "%% > tol " + String(MASS_TOL_REL * 100.0) + "%%")
