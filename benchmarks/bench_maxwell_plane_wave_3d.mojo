...
def main() raises:
    ...
    if rel_l2 > L2_MAX_REL:
        raise Error("bench_maxwell_plane_wave_3d FAILED: rel L2 " + String(rel_l2) + " exceeds threshold " + String(L2_MAX_REL))
    if max_zero_leak > ZERO_COMPONENT_MAX:
        raise Error(String("bench_maxwell_plane_wave_3d FAILED: zero-component ") + "leakage " + String(max_zero_leak) + " > " + String(ZERO_COMPONENT_MAX))
