...
def _run(NX: Int) raises -> Float64:
    ...
    if mass_rel > MASS_TOL_REL:
        raise Error(
            String("bench_shallow_water_wave_2d_p4 FAILED: mass drift ")
            + String(mass_rel)
            + " > tol "
            + String(MASS_TOL_REL)
            + " (at NX="
...
def main() raises:
    ...
    if err8 > L2_MAX_REL:
        raise Error(
            "bench_shallow_water_wave_2d_p4 FAILED: NX=8 rel L2 " + String(err8) + " exceeds " + String(L2_MAX_REL)
        )
    if err12 > L2_MAX_REL:
        raise Error(
            "bench_shallow_water_wave_2d_p4 FAILED: NX=12 rel L2 " + String(err12) + " exceeds " + String(L2_MAX_REL)
        )
    if err16 > L2_MAX_REL:
        raise Error(
            "bench_shallow_water_wave_2d_p4 FAILED: NX=16 rel L2 " + String(err16) + " exceeds " + String(L2_MAX_REL)
        )
