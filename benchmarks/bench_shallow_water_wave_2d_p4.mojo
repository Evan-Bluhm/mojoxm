# ======================================================================
# bench_shallow_water_wave_2d_p4 -- linear SW wave at P=4 (NP=15)
# ======================================================================
#
# P=4 counterpart of bench_shallow_water_wave_2d_p3.  Same small-
# amplitude linear shallow-water wave on a periodic-x domain,
# constant in y, but routed through LocalMesh2D[4] /
# sw_rk_stage_hll_2d[4] with NP=15 nodes per triangle.
#
# Closes the 2D SW P-parity gap up to P=4, mirroring the 2D Euler
# (`bench_euler_smooth_wave_2d_p4`) and 2D Maxwell
# (`bench_maxwell_plane_wave_2d_p4`) gates at the same NP=15.
#
# Pass criteria (P=4, sweep NX = 8, 12, 16, square cells, HLL flux):
#   * rel L2 at every NX < 5e-4
#   * mass conservation drift < 1e-4 (Float32 roundoff floor)
#   * no NaN / Inf
#
# At P=4 the dispersion error is well below the A/H=0.01 nonlinear
# correction floor (~1.7e-4 same as P=3), so this is an absolute-L2
# regression sentinel rather than a convergence-rate gate.
# ======================================================================
...
# At P=4 the rel L2 sits at the A/H=0.01 nonlinear correction floor
# across the entire sweep (~1.7e-4 expected, same as P=3).  5e-4 is
# ~3x the empirical floor; matches the P=3 threshold convention.
...
def _run(NX: Int) raises -> Float64:
    ...
    for k in range(n_q):
        ...
        if isnan(v) or isinf(v):
            raise Error("bench_shallow_water_wave_2d_p4: non-finite")
    ...
    if mass_rel > MASS_TOL_REL:
        raise Error(String("bench_shallow_water_wave_2d_p4 FAILED: mass drift ") + String(mass_rel) + " > tol " + String(MASS_TOL_REL) + " (at NX=" + String(NX) + ")")
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_shallow_water_wave_2d_p4: runs at np=1 only")
        return

    print("bench_shallow_water_wave_2d_p4 (linear SW wave, 2D HLL, P=4)")
    print("  P=", P, "  NP=", num_tri_nodes_2d(P), "  sweep NX=8, 12, 16   (threshold", L2_MAX_REL, ")")
    ...
    if err8 > L2_MAX_REL:
        raise Error("bench_shallow_water_wave_2d_p4 FAILED: NX=8 rel L2 " + String(err8) + " exceeds " + String(L2_MAX_REL))
    if err12 > L2_MAX_REL:
        raise Error("bench_shallow_water_wave_2d_p4 FAILED: NX=12 rel L2 " + String(err12) + " exceeds " + String(L2_MAX_REL))
    if err16 > L2_MAX_REL:
        raise Error("bench_shallow_water_wave_2d_p4 FAILED: NX=16 rel L2 " + String(err16) + " exceeds " + String(L2_MAX_REL))

    print("=== bench_shallow_water_wave_2d_p4 PASSED ===")
    mpi.finalize()
