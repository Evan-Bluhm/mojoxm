...
def main() raises:
    ...
    print("  P=", P, "  NP=", num_tri_nodes_2d(P),
          "  sweep NX=8, 12, 16   (threshold", L2_MAX_REL, ")")
    ...
    if err8 > L2_MAX_REL:
        raise Error(
            "bench_shallow_water_wave_2d_p4 FAILED: NX=8 rel L2 "
            + String(err8) + " exceeds " + String(L2_MAX_REL)
        )
    if err12 > L2_MAX_REL:
        raise Error(
            "bench_shallow_water_wave_2d_p4 FAILED: NX=12 rel L2 "
            + String(err12) + " exceeds " + String(L2_MAX_REL)
        )
    if err16 > L2_MAX_REL:
        raise Error(
            "bench_shallow_water_wave_2d_p4 FAILED: NX=16 rel L2 "
