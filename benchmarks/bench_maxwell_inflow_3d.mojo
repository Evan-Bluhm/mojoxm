...
def main() raises:
    ...
    var physics = Maxwell(
        ...
        EX0, EY0, EZ0, BX0, BY0, BZ0,              # inflow ghost
    )
    ...
    for i in range(n_owned_nodes):
        ...
        var by = q_ptr[i * 6 + 4]
        var bz = q_ptr[i * 6 + 5]
        if (isnan(qx) or isinf(qx) or isnan(qy) or isinf(qy)
            or isnan(qz) or isinf(qz) or isnan(bx) or isinf(bx)
        ...
        if d_ey < 0.0: d_ey = -d_ey
        var d_ez = Float64(qz - EZ0)
        if d_ez < 0.0: d_ez = -d_ez
        var d_bx = Float64(bx - BX0)
        if d_bx < 0.0: d_bx = -d_bx
        var d_by = Float64(by - BY0)
        if d_by < 0.0: d_by = -d_by
        var d_bz = Float64(bz - BZ0)
        if d_bz < 0.0: d_bz = -d_bz
        var local_max = d_ex
        if d_ey > local_max: local_max = d_ey
        if d_ez > local_max: local_max = d_ez
        if d_bx > local_max: local_max = d_bx
        if d_by > local_max: local_max = d_by
        if d_bz > local_max: local_max = d_bz
        if local_max > max_drift: max_drift = local_max
