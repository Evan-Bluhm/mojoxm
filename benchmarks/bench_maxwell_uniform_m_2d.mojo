# ======================================================================
# bench_maxwell_uniform_m_2d -- 2D Maxwell M source-term gate
# ======================================================================
#
# Companion to bench_maxwell_uniform_j_2d.  Same uniform-field
# analytic test as the J-source bench: periodic box, q = 0 IC, all
# flux divergences vanish, only the source contributes:
#
#   dBz/dt = -Mz   ->  Bz(T) = -Mz * T   (exact)
#   dE/dt = 0  identically (J = 0)
#
# Closes the M-arm of the 2D Maxwell source-term coverage that the
# previous commit added.  Pairs with bench_maxwell_uniform_m_3d for
# cross-dimension parity on the magnetic-current path.
#
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_maxwell_uniform_m_2d: runs at np=1 only")
        return

    print("bench_maxwell_uniform_m_2d (uniform M source-term gate)")
    ...
    print("  max |Bz - exact| / |Bz_exact| =", max_bz_rel, "  (threshold", BZ_REL_TOL, ")")
    print("  max |Ex,Ey,Ez,Bx,By| =", max_zero, "  (threshold", ZERO_COMPONENT_TOL, ")")

    if max_bz_rel > BZ_REL_TOL:
        raise Error(String("bench_maxwell_uniform_m_2d FAILED: Bz rel err ") + String(max_bz_rel) + " > " + String(BZ_REL_TOL))
    if max_zero > ZERO_COMPONENT_TOL:
        raise Error(String("bench_maxwell_uniform_m_2d FAILED: zero-component drift ") + String(max_zero) + " > " + String(ZERO_COMPONENT_TOL))
