# ======================================================================
# bench_maxwell_plane_wave_3d_p4 -- 3D TM plane wave at P=4 (NP=35)
# ======================================================================
#
# P=4 counterpart of bench_maxwell_plane_wave_3d_p3.  Same TM-mode
# plane wave on a triply-periodic [0, 1] x [0, NY/NX] x [0, NZ/NX]
# box, but routed through Mesh[4] / Solver[Maxwell, 4] with NP = 35
# nodes per tet.
#
# Pairs with bench_advection_3d_p4, bench_euler_smooth_wave_3d_p4
# (also NP=35 multi-component) to validate Maxwell at NP=35 in 3D.
#
...
#     dominate -- the P=4 dispersion error is well below this)
...
# At P=4 / NP=35 the dispersion error drops well below the
# Kuhn-tet asymmetry floor (observed ~3e-5).  5e-4 is ~15x margin
# but matches the P=3 threshold convention; tighter would risk
# false-firing on Float32 wobble across hardware.
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_maxwell_plane_wave_3d_p4: runs at np=1 only")
        return
    ...
    for k in range(n_owned_dof):
        ...
        if isnan(v_now) or isinf(v_now):
            raise Error("bench_maxwell_plane_wave_3d_p4: non-finite output")
    ...
    if max_zero_leak > ZERO_COMPONENT_MAX:
        raise Error(String("bench_maxwell_plane_wave_3d_p4 FAILED: zero-component ") + "leakage " + String(max_zero_leak) + " > " + String(ZERO_COMPONENT_MAX))

    print("=== bench_maxwell_plane_wave_3d_p4 PASSED ===")
    mpi.finalize()
