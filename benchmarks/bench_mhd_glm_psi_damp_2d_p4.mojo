# ======================================================================
# bench_mhd_glm_psi_damp_2d_p4 -- 2D GLM operator-splitting decay at P=4
# ======================================================================
#
# P=4 (NP=15) intermediate between bench_mhd_glm_psi_damp_2d_p3
# (NP=10) and bench_mhd_glm_psi_damp_2d_p5 (NP=21).  Same uniform
# rest IC + uniform psi=A0, same alpha_d>0 / c_h=0 setup, same
# operator-splitting damp kernel applied once per timestep, but
# routed through LocalMesh2D[4] / mhd_glm_rk_stage_2d[4] /
# launch_mhd_glm_psi_damp_2d[NP=15].
#
# Closes the 2D GLM damp P-parity hole.  Sweep is now P=2/P=3/P=4/P=5
# across both 2D and 3D for the GLM damp rate gate.  Pairs with
# bench_mhd_glm_psi_transport_2d_p4 for the full set of 2D MHD GLM
# P=4 rate gates.
#
# Pass criteria (P=4, sweep N=8, 12, T=1, alpha_d=1, A0=0.1):
#   * |psi(T) - A0/e| / A0 < 5e-5 at every owned node
#   * non-psi state unchanged from IC to ~5e-5 (Float32 step
#     accumulation budget; same as the P=5 analog)
#   * no NaN / Inf
# ======================================================================
...
# Looser than P=3's 2e-5 since at NX=12 P=4 we run ~720 steps vs
# P=3's ~560.  Drift is pure Float32 step accumulation on the
# constant non-psi state, scales with step count.
...
def _run(NX: Int) raises -> Bool:
    ...
    if rel_err > PSI_TOL_REL:
        raise Error(String("bench_mhd_glm_psi_damp_2d_p4 FAILED at NX=") + String(NX) + ": psi rel err " + String(rel_err) + " > tol " + String(PSI_TOL_REL))
    if max_state_err > STATE_TOL:
        raise Error(String("bench_mhd_glm_psi_damp_2d_p4 FAILED at NX=") + String(NX) + ": non-psi state drifted by " + String(max_state_err))
    return True
...
def main() raises:
    ...
    print("=== bench_mhd_glm_psi_damp_2d_p4 PASSED ===")
    mpi.finalize()
