...
def _run(NX: Int) raises -> Float64:
    ...
    if psi_max > amp_bound:
        raise Error(String("bench_mhd_glm_psi_transport_2d_p3 FAILED: psi_max ") + String(psi_max) + " exceeds 1.1 * AMPLITUDE " + String(amp_bound))
...
def main() raises:
    ...
    if err12 > L2_MAX_REL:
        raise Error("bench_mhd_glm_psi_transport_2d_p3 FAILED: NX=12 rel L2 " + String(err12) + " exceeds " + String(L2_MAX_REL))
    if err16 > L2_MAX_REL:
        raise Error("bench_mhd_glm_psi_transport_2d_p3 FAILED: NX=16 rel L2 " + String(err16) + " exceeds " + String(L2_MAX_REL))
    if err24 > L2_MAX_REL:
        raise Error("bench_mhd_glm_psi_transport_2d_p3 FAILED: NX=24 rel L2 " + String(err24) + " exceeds " + String(L2_MAX_REL))
