# ======================================================================
# bench_mhd_glm_psi_damp_3d_p3 -- 3D GLM psi damping at P=3
# ======================================================================
#
# P=3 (NP=20 nodes per tet) counterpart of bench_mhd_glm_psi_damp_3d.
# Same uniform IC (rho=1, p=1, u=v=w=0, B=(B0, 0, 0), psi=A0), same
...
# bench_mhd_alfven_3d_p3 (full GLM Alfven wave) for complete 3D
# MHD P=3 coverage of the GLM stack:
#   transport (c_h>0, alpha_d=0) | damp (c_h=0, alpha_d>0) | both
# all gated at NP=10 (2D) and NP=20 (3D).
#
# Pass criteria (P=3, periodic 4x4x4, T=1, alpha_d=1, A0=0.1):
...
def damp_ic_kernel_p3(q: UnsafePointer[Float32, MutAnyOrigin], owned_elem_ids: UnsafePointer[Int32, MutAnyOrigin], elem_node_xyz: UnsafePointer[Float32, MutAnyOrigin], num_owned: Int):
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_mhd_glm_psi_damp_3d_p3: runs at np=1 only")
        return

    print("bench_mhd_glm_psi_damp_3d_p3 (3D GLM psi damping at P=3)")
    ...
    # Build P=3 reference operators directly.
    ...
    solver.ctx.enqueue_function[damp_ic_kernel_p3](
    ...
    for i in range(n_owned_nodes):
        for c in range(9):
            ...
            if isnan(v) or isinf(v):
                raise Error("bench_mhd_glm_psi_damp_3d_p3: non-finite output")
            if c == 8:
    ...
    if psi_rel > PSI_TOL_REL:
        raise Error("bench_mhd_glm_psi_damp_3d_p3 FAILED: psi rel err " + String(psi_rel) + " > " + String(PSI_TOL_REL))
    if max_state_drift > STATE_TOL:
        raise Error("bench_mhd_glm_psi_damp_3d_p3 FAILED: state drift " + String(max_state_drift) + " > " + String(STATE_TOL))

    print("=== bench_mhd_glm_psi_damp_3d_p3 PASSED ===")
    mpi.finalize()
