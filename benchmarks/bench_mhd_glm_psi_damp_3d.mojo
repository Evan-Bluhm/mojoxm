...
def damp_ic_kernel(q: UnsafePointer[Float32, MutAnyOrigin], owned_elem_ids: UnsafePointer[Int32, MutAnyOrigin], elem_node_xyz: UnsafePointer[Float32, MutAnyOrigin], num_owned: Int):
...
def main() raises:
    ...
    print("  P= 2   mesh=", NX, "x", NY, "x", NZ, "   alpha_d=", ALPHA_D, "   T=", T_FINAL)
    ...
    solver.ctx.enqueue_function[damp_ic_kernel](
    ...
    if psi_rel > PSI_TOL_REL:
        raise Error("bench_mhd_glm_psi_damp_3d FAILED: psi rel err " + String(psi_rel) + " > " + String(PSI_TOL_REL))
    if max_state_drift > STATE_TOL:
        raise Error("bench_mhd_glm_psi_damp_3d FAILED: state drift " + String(max_state_drift) + " > " + String(STATE_TOL))
