# ======================================================================
# bench_mhd_alfven_glm_2d_p5 -- Alfven via GLM 2D MHD at P=5 (NP=21)
# ======================================================================
#
# P=5 counterpart of bench_mhd_alfven_glm_2d_p4.  Same Alfven-wave IC
# and the same GLM-with-c_h=0 setup (so the GLM path reduces to plain
# ideal MHD on a smooth div(B)=0 IC), but routed through
# LocalMesh2D[5] / mhd_glm_rk_stage_2d[5] with NP=21 nodes per
# triangle.
#
# Closes the 2D MHD GLM (NC=7) P-parity sweep at P=5.  Highest-NC
# 2D physics gate at NP=21 -- exercises the comptime template at
# NC=7 / NP=21 = 147 q values per element.  Together with the
# already-landed Euler / Maxwell / SW / advection 2D P=5 gates this
# brings full 2D NP=21 P=5 coverage across all 5 physics paths.
#
# Pass criteria (P=5, NX=24, thin y-strip):
#   * rel L2(state, including psi) < 5e-3 (same A^2 nonlinear
#     correction floor as P=2/3/4 -- IC sets the floor, not the
#     scheme)
...
def _run() raises -> Float64:
    ...
    for k in range(n_q):
        ...
        if isnan(v) or isinf(v):
            raise Error("bench_mhd_alfven_glm_2d_p5: non-finite at index " + String(k))
    ...
    var rel_l2 = l2 / l2_ic
    if psi_max > PSI_TOL:
        raise Error(String("bench_mhd_alfven_glm_2d_p5 FAILED: psi_max ") + String(psi_max) + " > tol " + String(PSI_TOL))
    return rel_l2
...
def main() raises:
    ...
    mpi.init()
    print("bench_mhd_alfven_glm_2d_p5 (Alfven via GLM 2D MHD, P=5, c_h=0)")
    print("  P=", P, "  NP=", num_tri_nodes_2d(P), "  NX=", NX)

    var rel_l2 = _run()
    print("  rel L2(state) =", rel_l2, "  (threshold", L2_MAX_REL, ")")
    if rel_l2 > L2_MAX_REL:
        raise Error("bench_mhd_alfven_glm_2d_p5 FAILED: rel L2 " + String(rel_l2) + " exceeds threshold " + String(L2_MAX_REL))
    print("=== bench_mhd_alfven_glm_2d_p5 PASSED ===")
    mpi.finalize()
