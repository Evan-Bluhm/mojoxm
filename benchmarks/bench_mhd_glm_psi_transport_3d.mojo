...
def transport_ic_kernel(q: UnsafePointer[Float32, MutAnyOrigin], owned_elem_ids: UnsafePointer[Int32, MutAnyOrigin], elem_node_xyz: UnsafePointer[Float32, MutAnyOrigin], num_owned: Int):
...
def main() raises:
    ...
    print("  P= 2   mesh=", NX, "x", NY, "x", NZ, "   c_h=", C_H, "   T=", T_FINAL, "   amplitude=", AMPLITUDE)
    ...
    solver.ctx.enqueue_function[transport_ic_kernel](
    ...
    if psi_max > amp_bound:
        raise Error(String("bench_mhd_glm_psi_transport_3d FAILED: psi_max ") + String(psi_max) + " exceeds 1.1 * AMPLITUDE " + String(amp_bound))
