# ======================================================================
# bench_mhd_inflow_2d -- first BC gate for 2D plain (NC=6) MHD
# ======================================================================
#
# Plain-MHD analog of bench_mhd_inflow_2d_glm.  The 2D plain-MHD
# face-flux kernel previously handled only BC_INTERIOR + BC_WALL with
# the else branch acting as zero-gradient outflow; BC_INFLOW had no
# handler.  This iteration adds the BC_INFLOW path to
# mhd_face_flux_kernel_2d (NC=6, no psi) and closes the loop with
# this bench.
#
# Setup mirrors bench_mhd_inflow_2d_glm: matched-state subsonic
# inflow on -x with BC_OUTFLOW on +x and periodic in y.  IC = inflow
# ghost so the analytic solution is the IC unchanged.  No GLM in
# the plain-MHD path so no BC-coupling caveat applies.
#
# Pass criteria (P=2, NX=32 NY=4, T=1):
#   * max relative drift in any of the 6 components < 5e-3
#   * no NaN / Inf
# ======================================================================
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_mhd_inflow_2d: runs at np=1 only")
        return

    print("bench_mhd_inflow_2d (BC_INFLOW preservation, plain MHD NC=6)")
    print("  P=", P, "  mesh=", NX, "x", NY, "   U0=", U0, "   B0=", B0, "   T=", T_FINAL)
    ...
    # IC: uniform (rho, rhou, rhov, Bx, By, E) matched to the inflow ghost.
    # E = p/(g-1) + 0.5*rho*|u|^2 + 0.5*|B|^2.
    ...
    if max_drift > REL_TOL:
        raise Error("bench_mhd_inflow_2d FAILED: max relative drift " + String(max_drift) + " > " + String(REL_TOL))
