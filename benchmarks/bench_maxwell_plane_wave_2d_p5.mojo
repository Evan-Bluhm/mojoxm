# ======================================================================
# bench_maxwell_plane_wave_2d_p5 -- 2D TM plane wave at P=5 (NP=21)
# ======================================================================
#
# P=5 counterpart of bench_maxwell_plane_wave_2d_p4.  Same periodic
# [0, 1]^2 box, same TM-mode plane wave traveling in +x at speed c
# (Ez = cos(2 pi (x - c t)), By = -(1/c) cos(2 pi (x - c t))), but
# routed through `LocalMesh2D[5]` / `maxwell_rk_stage_2d[5]` with
# NP=21 nodes per triangle.
#
# Highest-order multi-component vacuum-Maxwell gate.  Mirrors the
# P=5 Euler entropy-wave (`bench_euler_smooth_wave_2d_p5`, also
# NP=21) and P=5 advection (`bench_advection_translation_2d_p5`,
# NP=21 single-component) for cross-physics P=5 / NP=21 coverage.
# At NP=21 in 2D the per-element register pressure and operator
# size hit comptime extremes that earlier benches don't reach;
# this gate is also a structural smoke test for the high-P EM
# operator construction (Vandermonde inverse, Lift_ref, mass-matrix
# weights) on the linear central-flux Maxwell path.
#
# Pass criteria (P=5, NX=NY=6, single-rank, periodic):
#   * rel L2(state) at one period < 2e-4 (Float32-floor regime;
#     observed ~1.5e-5)
#   * leakage into analytically-zero components (Ex, Ey, Bx, Bz)
#     bounded < 2e-4 (Kuhn-tet asymmetry floor, P-independent)
#   * no NaN / Inf
# ======================================================================

from std.math import sqrt, cos, pi, isnan, isinf
...
# At P=5 / NP=21 the dispersion error is well below the Kuhn-tet
# asymmetry floor (observed ~1.5e-5).  2e-4 matches the P=3/P=4
# convention -- catches any P=5 Maxwell regression without false-
# firing on Float32 jitter.
...
def main() raises:
    ...
    # IC: Ez = cos(2 pi x), By = -(1/c) cos(2 pi x), rest 0.
    var two_pi = 2.0 * pi
