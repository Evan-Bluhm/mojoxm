...
def _run(N: Int) raises -> Float64:
    ...
    for k in range(n_q):
        ...
        if isnan(v) or isinf(v):
            raise Error("bench_euler_vortex_2d: non-finite output at index " + String(k))
...
def main() raises:
    ...
    if rel_l2 > L2_MAX_REL:
        raise Error("bench_euler_vortex_2d FAILED: rel L2 " + String(rel_l2) + " exceeds threshold " + String(L2_MAX_REL))
