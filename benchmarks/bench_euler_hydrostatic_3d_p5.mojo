# ======================================================================
# bench_euler_hydrostatic_3d_p5 -- 3D Euler gravity at P=5 (NP=56)
# ======================================================================
#
# P=5 (NP=56 nodes per tet) counterpart of bench_euler_hydrostatic_3d_p3.
# Same constant-density hydrostatic-balance setup -- gravity g =
# (0, 0, -|gz|), p(z) = p0 - rho0*|gz|*z, slip walls in z, periodic
# in x/y -- but routed through Mesh[5] / Solver[Euler, 5] /
# rk_stage_kernel[5] so the 3D Euler gravity source-term path is
# exercised at NP=56 -- the highest 3D Euler NP the bench harness
# gates end-to-end.  Mesh shrunk to 2x2x4 since per-element volume
# work scales with NP^2 and dt tightens to 1/(2P+1)=1/11 at P=5.
#
# Pass criteria (P=5, NX=NY=2, NZ=4, single-rank, T=1):
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_euler_hydrostatic_3d_p5: runs at np=1 only")
        return
    ...
    for i in range(n_owned_nodes):
        ...
        if isnan(rho_now) or isinf(rho_now):
            raise Error("bench_euler_hydrostatic_3d_p5: non-finite output")
        var u = rhou / rho_now
        var v = rhov / rho_now
        var w = rhow / rho_now
    ...
    if max_rho_dev > RHO_REL_TOL:
        raise Error(
            "bench_euler_hydrostatic_3d_p5 FAILED: max drho/rho0 " + String(max_rho_dev) + " > " + String(RHO_REL_TOL)
        )
