# ======================================================================
# bench_euler_hydrostatic_3d_p3 -- 3D Euler gravity at P=3 (NP=20)
# ======================================================================
#
# P=3 (NP=20 nodes per tet) counterpart of bench_euler_hydrostatic_3d.
# Same constant-density hydrostatic-balance setup -- gravity g =
# (0, 0, -|gz|), p(z) = p0 - rho0*|gz|*z, slip walls in z, periodic
# in x/y -- but routed through Mesh[3] / Solver[Euler, 3] /
# rk_stage_kernel[3] so the 3D Euler gravity source-term path is
# exercised at NP=20.
#
# Mirrors bench_euler_hydrostatic_2d_p3 (NP=10 in 2D); together with
# the existing P=2 variants the gravity source path now has full
# 4-corner P / dimension parity.
#
# Pass criteria (P=3, NX=NY=4, NZ=8, single-rank, T=1):
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_euler_hydrostatic_3d_p3: runs at np=1 only")
        return

    print("bench_euler_hydrostatic_3d_p3 (hydrostatic balance at P=3, NP=20)")
    print(
        "  P=",
        P,
        "  NP=",
    ...
    var hbuf_xyz = solver.ctx.enqueue_create_host_buffer[DType.float32](solver.num_owned_elements * NP * 3)
    ...
    var num_xyz = solver.mesh.local.num_elements * NP * 3
    var hbuf_xyz_full = solver.ctx.enqueue_create_host_buffer[DType.float32](num_xyz)
    solver.ctx.enqueue_copy(
        hbuf_xyz_full,
        solver.mesh.local.d_elem_node_xyz.create_sub_buffer[DType.float32](0, num_xyz),
    )
    solver.ctx.synchronize()
    ...
    for i in range(n_owned_nodes):
        ...
        if isnan(rho_now) or isinf(rho_now):
            raise Error("bench_euler_hydrostatic_3d_p3: non-finite output")
        var u = rhou / rho_now
        var v = rhov / rho_now
        var w = rhow / rho_now
        ...
        var owned_e = Int(
            solver.mesh.d_owned_elem_ids.create_sub_buffer[DType.int32](0, solver.num_owned_elements).unsafe_ptr()[0]
        )
    ...
    if max_v > VMAX_TOL:
        raise Error("bench_euler_hydrostatic_3d_p3 FAILED: max |v| " + String(max_v) + " > " + String(VMAX_TOL))
    if max_rho_dev > RHO_REL_TOL:
        raise Error(
            "bench_euler_hydrostatic_3d_p3 FAILED: max drho/rho0 " + String(max_rho_dev) + " > " + String(RHO_REL_TOL)
        )
    if max_p_dev > P_REL_TOL:
        raise Error("bench_euler_hydrostatic_3d_p3 FAILED: max dp/p0 " + String(max_p_dev) + " > " + String(P_REL_TOL))

    print("=== bench_euler_hydrostatic_3d_p3 PASSED ===")
    mpi.finalize()
