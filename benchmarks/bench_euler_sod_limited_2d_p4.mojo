...
def main() raises:
    ...
    if err_L > PLATEAU_L_TOL_REL:
        raise Error(String("bench_euler_sod_limited_2d_p4 FAILED: left plateau err ") + String(err_L))
    var err_R = (meas_R - rho_ref_R) / rho_ref_R
    if err_R < 0.0:
        err_R = -err_R
    if err_R > PLATEAU_R_TOL_REL:
        raise Error(String("bench_euler_sod_limited_2d_p4 FAILED: right plateau err ") + String(err_R))
    var err_star = (meas_star - rho_ref_star) / rho_ref_star
    if err_star < 0.0:
        err_star = -err_star
    if err_star > STAR_TOL_REL:
        raise Error(String("bench_euler_sod_limited_2d_p4 FAILED: star plateau err ") + String(err_star))
    ...
    if a_shock_err_cells > SHOCK_TOL_CELLS:
        raise Error(String("bench_euler_sod_limited_2d_p4 FAILED: shock position off by ") + String(a_shock_err_cells) + " cells (tol " + String(SHOCK_TOL_CELLS) + ")")
