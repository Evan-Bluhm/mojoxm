# ======================================================================
# bench_two_fluid_walls_3d_p5 -- 3D Two-Fluid BC_WALL preservation at P=5
# ======================================================================
#
# P=5 (NP=56 nodes per tet) counterpart of bench_two_fluid_walls_3d_p3.
# Same shrunk 2x2x2 mesh as the P=4 variant -- per-element work scales
# with NP^2 (~9x P=3's at P=5) and dt tightens to 1/(2P+1)=1/11.  This
# is the highest 3D Two-Fluid NP the bench harness gates end-to-end,
# completing P-parity at NC=17 in 3D.
#
# Pass criteria (P=5, NX=NY=NZ=2, T=0.5):
#   * max |q - q_IC| < 5e-4
#   * no NaN / Inf
# ======================================================================
...
def main() raises:
    ...
    for k in range(n_owned_dof):
        ...
        if isnan(v) or isinf(v):
            raise Error("bench_two_fluid_walls_3d_p5: non-finite output")
