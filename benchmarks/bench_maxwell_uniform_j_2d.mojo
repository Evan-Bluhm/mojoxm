...
def main() raises:
    ...
    if max_ex_rel > EX_REL_TOL:
        raise Error(String("bench_maxwell_uniform_j_2d FAILED: Ex rel err ") + String(max_ex_rel) + " > " + String(EX_REL_TOL))
    if max_zero > ZERO_COMPONENT_TOL:
        raise Error(String("bench_maxwell_uniform_j_2d FAILED: zero-component drift ") + String(max_zero) + " > " + String(ZERO_COMPONENT_TOL))
