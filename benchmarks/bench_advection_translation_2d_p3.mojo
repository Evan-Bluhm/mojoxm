# ======================================================================
# bench_advection_translation_2d_p3 -- 2D Gaussian advection at P=3
# ======================================================================
#
# P=3 counterpart of bench_advection_translation_2d.  Same Gaussian-
# bump one-period advection problem on a periodic [0, 1]^2 mesh, but
# routed through LocalMesh2D[3] / ReferenceElement2D[3] /
# advection_rk_stage_2d[3] (the parameterized 2D triangular path).
# Expected asymptotic convergence rate is P+1 = 4.
#
# Validates that the 2D triangular pipeline's NP=10 (P=3 triangle)
# operators -- D_ref, Lift_ref, node_weights -- produce design-rate
# accuracy.  Counterpart to bench_advection_3d_p3 for the 2D stack.
#
# Pass criteria (P=3, SSPRK3, sweep N = 8, 12, 16):
#   * rel L2 at N=16 < 5e-4 (Gaussian well-resolved at P=3)
#   * monotone refinement
#   * rate >= 2.5 between at least one consecutive pair
#     (theoretical P+1=4; pre-asymptotic floor)
# ======================================================================
...
def _run(N: Int) raises -> Float64:
    ...
    for k in range(n_q):
        ...
        if isnan(v) or isinf(v):
            raise Error("bench_advection_translation_2d_p3: non-finite output")
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_advection_translation_2d_p3: runs at np=1 only")
        return
    ...
    if err16 > L2_MAX_REL_AT_16:
        raise Error("bench_advection_translation_2d_p3 FAILED: rel L2 at N=16 " + String(err16) + " exceeds " + String(L2_MAX_REL_AT_16))
    if not (err8 > err12 and err12 > err16):
        raise Error("bench_advection_translation_2d_p3 FAILED: L2 not monotone" + " (8: " + String(err8) + ", 12: " + String(err12) + ", 16: " + String(err16) + ")")

    var rate_812 = log(err8 / err12) / log(12.0 / 8.0)
    var rate_1216 = log(err12 / err16) / log(16.0 / 12.0)
    print("  observed rates: log_(3/2)(e8/e12) =", rate_812, "  log_(4/3)(e12/e16) =", rate_1216, "  (P+1 =", P + 1, ", floor", RATE_MIN, ")")
    if rate_812 < RATE_MIN and rate_1216 < RATE_MIN:
        raise Error(String("bench_advection_translation_2d_p3 FAILED: rates ") + String(rate_812) + " and " + String(rate_1216) + " both below " + String(RATE_MIN))
