# ======================================================================
# bench_mhd_glm_psi_damp_2d_p5 -- 2D GLM operator-splitting decay at P=5
# ======================================================================
#
# P=5 (NP=21) counterpart of bench_mhd_glm_psi_damp_2d_p3.  Same uniform
# rest IC + uniform psi=A0, same alpha_d>0 / c_h=0 setup, same operator-
# splitting damp kernel applied once per timestep, but routed through
# LocalMesh2D[5] / mhd_glm_rk_stage_2d[5] / launch_mhd_glm_psi_damp_2d
# [NP=21] so the NC=7 GLM stack is exercised at the highest 2D NP.
#
# Companion to bench_mhd_glm_psi_damp_3d_p5 (the 3D source-term-hook
# decay-rate gate at NP=56).  Together they pin SSPRK3's per-step
# `exp(-alpha_d*dt)` operator-split multiplier composing to
# `exp(-alpha_d*T)` correctly at the highest P in both dimensions.
#
# Pass criteria (P=5, sweep N=8, 12, T=1, alpha_d=1, A0=0.1):
#   * |psi(T) - A0/e| / A0 < 5e-5 at every owned node
...
# Looser than P=3's 2e-5 since at NX=12 P=5 we run ~880 steps vs P=3's
# ~560.  Drift is pure Float32 step accumulation on the constant non-
# psi state, scales with step count.
comptime STATE_TOL: Float32 = Float32(5.0e-5)
...
def _run(NX: Int) raises -> Bool:
    ...
    var host_mesh = LocalMesh2D[P](Nx=NX, Ny=NY, Lx=LX, Ly=LY_run)
    ...
    for i in range(n_nodes):
        ...
        if isnan(psi) or isinf(psi):
            raise Error("bench_mhd_glm_psi_damp_2d_p5: non-finite psi")
    ...
    if rel_err > PSI_TOL_REL:
        raise Error(String("bench_mhd_glm_psi_damp_2d_p5 FAILED at NX=") + String(NX) + ": psi rel err " + String(rel_err) + " > tol " + String(PSI_TOL_REL))
    if max_state_err > STATE_TOL:
        raise Error(String("bench_mhd_glm_psi_damp_2d_p5 FAILED at NX=") + String(NX) + ": non-psi state drifted by " + String(max_state_err))
    return True
...
def main() raises:
    ...
    mpi.init()
    print("bench_mhd_glm_psi_damp_2d_p5 (GLM operator-splitting decay at P=5)")
    print("  P=", P, "  alpha_d=", ALPHA_D, "  T=", T_FINAL, "  expected psi/A0 = exp(-alpha_d*T) = ", exp(-Float64(ALPHA_D) * Float64(T_FINAL)))

    _ = _run(8)
    _ = _run(12)

    print("=== bench_mhd_glm_psi_damp_2d_p5 PASSED ===")
    mpi.finalize()
