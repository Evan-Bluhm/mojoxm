...
def main() raises:
    ...
    if err12 > L2_MAX_REL:
        raise Error(
            "bench_euler_smooth_wave_3d FAILED: N=12 rel L2 " + String(err12) + " exceeds " + String(L2_MAX_REL)
        )
    if err16 > L2_MAX_REL:
        raise Error(
            "bench_euler_smooth_wave_3d FAILED: N=16 rel L2 " + String(err16) + " exceeds " + String(L2_MAX_REL)
        )
