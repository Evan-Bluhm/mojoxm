# ======================================================================
# bench_maxwell_plane_wave_2d_p4 -- 2D TM plane wave at P=4 (NP=15)
# ======================================================================
#
# P=4 counterpart of bench_maxwell_plane_wave_2d_p3.  Same periodic
...
#
# Routed through `LocalMesh2D[4]` / `maxwell_rk_stage_2d[4]` with
# NP=15 nodes per triangle.  Closes the Maxwell P-parity gap in
# 2D up to P=4, mirroring `bench_advection_translation_2d_p4`
# (NP=15 single-component) and `bench_euler_smooth_wave_2d_p4`
# (NP=15 multi-component HLLC).
#
# Pass criteria (P=4, NX=NY=8, single-rank, periodic):
#   * rel L2(state) at one period < 2e-4 (Float32-floor regime
#     at NP=15 -- the Maxwell central-flux dispersion error is
#     well below this; residual is dominated by Kuhn-tet
#     triangulation asymmetry under Rusanov + Float32 step
#     accumulation over O(100) SSPRK3 steps)
#   * leakage into analytically-zero components (Ex, Ey, Bx, Bz)
#     bounded < 2e-4 (same triangulation-asymmetry floor)
#   * no NaN / Inf
# ======================================================================
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_maxwell_plane_wave_2d_p4: runs at np=1 only")
        return

    print("bench_maxwell_plane_wave_2d_p4 (2D TM plane wave, P=4, periodic)")
    ...
    if max_zero_leak > ZERO_COMPONENT_MAX:
        raise Error(String("bench_maxwell_plane_wave_2d_p4 FAILED: zero-component ") + "leakage " + String(max_zero_leak) + " > " + String(ZERO_COMPONENT_MAX))

    print("=== bench_maxwell_plane_wave_2d_p4 PASSED ===")
    mpi.finalize()
