# ======================================================================
# bench_euler_smooth_wave_3d_p5 -- P=5 entropy-wave Euler (NP=56)
# ======================================================================
#
# P=5 counterpart of bench_euler_smooth_wave_3d_p4.  Same 3D entropy
# wave (uniform velocity + uniform pressure + sinusoidal density) on
# a triply-periodic [0, 1]^3 cube, but routed through Mesh[5] +
# Solver[Euler, 5] with NP = 56 nodes per tet.  Pairs with
# bench_advection_3d_p5 (NP=56, single-component) to cover NP=56
# operator correctness across single- and multi-component physics.
#
# At P=5 / NP=56 the entropy wave is well over-resolved on these
# meshes: errors sit at the Float32 round-off floor (~5e-5) across
# the full sweep, so this gate is an absolute-L2 sentinel rather
# than a rate test.  A regression in 3D mass-matrix inversion or
# Vandermonde quadrature at P=5 would push the error well above
# threshold.
#
# Pass criteria (P=5, HLLEC, periodic, single-rank, sweep N=4, 6, 8):
...
# Float32-floor regime across the full sweep at NP=56; 2e-4 is ~4x
# margin and catches any HLLEC P=5 regression that meaningfully
...
def _run(N: Int) raises -> Float64:
    ...
    for k in range(n_owned_dof):
        ...
        if isnan(v) or isinf(v):
            raise Error("bench_euler_smooth_wave_3d_p5: non-finite output")
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("bench_euler_smooth_wave_3d_p5: runs at np=1 only")
        return

    print("bench_euler_smooth_wave_3d_p5 (P=5 entropy wave, HLLEC)")
    print("  P=", P, "  NP=", NP, "  sweep N=4, 6, 8   (threshold",
          L2_MAX_REL, ")")
