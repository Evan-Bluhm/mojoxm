...
def main() raises:
    ...
    if p1.cx0 != 0 or p1.cx1 != 4 or p1.cy0 != 0 or p1.cy1 != 4 or p1.cz0 != 0 or p1.cz1 != 4:
        raise Error("partition_test FAILED: nprocs=1 owned range wrong")
    if p1.neighbour_minus_x != 0 or p1.neighbour_plus_x != 0 or p1.neighbour_minus_y != 0 or p1.neighbour_plus_y != 0 or p1.neighbour_minus_z != 0 or p1.neighbour_plus_z != 0:
    ...
    if p2_r0.px != 1 or p2_r0.py != 1 or p2_r0.pz != 2:
        raise Error("partition_test FAILED: nprocs=2 4x4x4 expected (1,1,2), got (" + String(p2_r0.px) + "," + String(p2_r0.py) + "," + String(p2_r0.pz) + ")")
    ...
    if p2_r0.neighbour_plus_z != 1 or p2_r0.neighbour_minus_z != 1:
        raise Error("partition_test FAILED: nprocs=2 r0 z-neighbours expected 1, got " + String(p2_r0.neighbour_plus_z) + "/" + String(p2_r0.neighbour_minus_z))
    # x and y neighbours of rank 0 are itself (px=py=1).
    if p2_r0.neighbour_minus_x != 0 or p2_r0.neighbour_plus_x != 0 or p2_r0.neighbour_minus_y != 0 or p2_r0.neighbour_plus_y != 0:
    ...
    if p3_r0.px != 2 or p3_r0.py != 2 or p3_r0.pz != 2:
        raise Error("partition_test FAILED: nprocs=8 4x4x4 expected (2,2,2), got (" + String(p3_r0.px) + "," + String(p3_r0.py) + "," + String(p3_r0.pz) + ")")
    ...
    if total != 64:
        raise Error("partition_test FAILED: nprocs=8 union != 64, got " + String(total))
    ...
    if p3_r0.neighbour_minus_x != 4 or p3_r0.neighbour_plus_x != 4:
        raise Error("partition_test FAILED: nprocs=8 r0 x-neighbour expected 4, got " + String(p3_r0.neighbour_minus_x) + "/" + String(p3_r0.neighbour_plus_x))
    if p3_r0.neighbour_minus_y != 2 or p3_r0.neighbour_plus_y != 2:
        raise Error("partition_test FAILED: nprocs=8 r0 y-neighbour expected 2, got " + String(p3_r0.neighbour_minus_y) + "/" + String(p3_r0.neighbour_plus_y))
    if p3_r0.neighbour_minus_z != 1 or p3_r0.neighbour_plus_z != 1:
        raise Error("partition_test FAILED: nprocs=8 r0 z-neighbour expected 1, got " + String(p3_r0.neighbour_minus_z) + "/" + String(p3_r0.neighbour_plus_z))
    ...
    if p4_r0.px != 1 or p4_r0.py != 2 or p4_r0.pz != 2:
        raise Error("partition_test FAILED: nprocs=4 4x4x4 expected (1,2,2), got (" + String(p4_r0.px) + "," + String(p4_r0.py) + "," + String(p4_r0.pz) + ")")
    if p4_r0.nx != 4 or p4_r0.ny != 2 or p4_r0.nz != 2:
        raise Error("partition_test FAILED: nprocs=4 r0 owned counts != (4,2,2)")
    ...
    if total4 != 64:
        raise Error("partition_test FAILED: nprocs=4 union != 64, got " + String(total4))
    ...
    if p4_r0.neighbour_minus_y != 2 or p4_r0.neighbour_plus_y != 2:
        raise Error("partition_test FAILED: nprocs=4 r0 y-neighbour expected 2, got " + String(p4_r0.neighbour_minus_y) + "/" + String(p4_r0.neighbour_plus_y))
    if p4_r0.neighbour_minus_z != 1 or p4_r0.neighbour_plus_z != 1:
        raise Error("partition_test FAILED: nprocs=4 r0 z-neighbour expected 1, got " + String(p4_r0.neighbour_minus_z) + "/" + String(p4_r0.neighbour_plus_z))
    ...
    try:
        ...
        raised = True
    if not raised:
        raise Error("partition_test FAILED: nprocs=3 / 4x4x4 should raise (no integer factorisation); did not raise")
    ...
    for nprocs in [1, 2, 4, 8]:
        for r in range(nprocs):
            ...
            # Walk -d then +d.
            var p_mx = build_partition(rank=Int(nmx), nprocs=nprocs, nx=4, ny=4, nz=4)
            if Int(p_mx.neighbour_plus_x) != r:
                raise Error("partition_test FAILED: nprocs=" + String(nprocs) + " r=" + String(r) + " (-x then +x) != self (" + String(p_mx.neighbour_plus_x) + ")")
            var p_px = build_partition(rank=Int(npx), nprocs=nprocs, nx=4, ny=4, nz=4)
            if Int(p_px.neighbour_minus_x) != r:
                raise Error("partition_test FAILED: nprocs=" + String(nprocs) + " r=" + String(r) + " (+x then -x) != self")
            var p_my = build_partition(rank=Int(nmy), nprocs=nprocs, nx=4, ny=4, nz=4)
            if Int(p_my.neighbour_plus_y) != r:
                raise Error("partition_test FAILED: nprocs=" + String(nprocs) + " r=" + String(r) + " (-y then +y) != self")
            var p_py = build_partition(rank=Int(npy), nprocs=nprocs, nx=4, ny=4, nz=4)
            if Int(p_py.neighbour_minus_y) != r:
                raise Error("partition_test FAILED: nprocs=" + String(nprocs) + " r=" + String(r) + " (+y then -y) != self")
            var p_mz = build_partition(rank=Int(nmz), nprocs=nprocs, nx=4, ny=4, nz=4)
            if Int(p_mz.neighbour_plus_z) != r:
                raise Error("partition_test FAILED: nprocs=" + String(nprocs) + " r=" + String(r) + " (-z then +z) != self")
            var p_pz = build_partition(rank=Int(npz), nprocs=nprocs, nx=4, ny=4, nz=4)
