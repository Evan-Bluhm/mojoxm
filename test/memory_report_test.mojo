...
def check[P: Int](mut nvtx: NvtxContext, rank: Int, size: Int) raises:
    ...
    var mesh = Mesh[P](ctx=ctx, part=build_partition(rank=rank, nprocs=size, nx=4, ny=4, nz=4), Lx=1.0, Ly=1.0, Lz=1.0, bcs=BoundaryConditions.periodic())
    ...
    var sum_check = rep.rk_stage_bytes + rep.dg_operators_bytes + rep.limiter_bytes + rep.mesh_connectivity_bytes + rep.halo_device_bytes
    if sum_check != rep.total_device_bytes():
        raise Error("memory_report_test P=" + String(P) + ": total_device_bytes (" + String(rep.total_device_bytes()) + ") != per-category sum (" + String(sum_check) + ")")
    ...
    if rep.rk_stage_bytes != nominal_rk:
        raise Error("memory_report_test P=" + String(P) + ": rk_stage_bytes " + String(rep.rk_stage_bytes) + " != nominal 3*num_owned*NP*sizeof(F32) = " + String(nominal_rk))
    ...
    if solver.dof_count() != expected_dof:
        raise Error("memory_report_test P=" + String(P) + ": dof_count " + String(solver.dof_count()) + " != expected " + String(expected_dof))

    var dt = Float32(1.0e-3)
    var tput = solver.bench_step_loop(dt, nvtx, warmup_steps=2, measure_steps=10)
    ...
    if bw > 1.0e12:
        raise Error("memory_report_test P=" + String(P) + ": state_bandwidth " + String(bw) + " B/s exceeds 1 TB/s sanity ceiling -- timing likely missing a ctx.synchronize()")
    ...
    if solver.state_bytes_per_step() != expected_state_bytes:
        raise Error("memory_report_test P=" + String(P) + ": state_bytes_per_step " + String(solver.state_bytes_per_step()) + " != expected " + String(expected_state_bytes))
