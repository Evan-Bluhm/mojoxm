# ======================================================================
# limiter_2d_gpu_test_p3 -- P=3 BJ limiter smoke test (NP=10)
# ======================================================================
#
# P=3 counterpart of limiter_2d_gpu_test.  The mass-matrix-weighted
# cell mean (`launch_cell_mean_2d` in `local_mesh_2d_gpu`) is what
# `bj_limit_full_2d` reads as the per-element mean -- the unweighted
# nodal arithmetic mean is wrong at P>=2 (different node-weight
# distributions per P).  At P=3 that distribution differs from P=2,
# so a regression in either the kernel weights or the BJ limiter's
# downstream use of them would not be caught by the existing P=2
# test.
#
# Same two checks as the P=2 test:
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("limiter_2d_gpu_test_p3: runs at np=1 only")
        return
    print("limiter_2d_gpu_test_p3 (P=3 / NP=10 BJ limiter)")
    ...
    # below 5 (the cell mean stays near 1 because P=3 has 9 other
    # nodes weighing in via the mass-matrix-weighted mean).  The
    # threshold (max rho < 2.5) holds for both P=2 and P=3 because
    # the BJ scaling pulls the spike toward the cell mean either
    # way -- only the precise post-limit value differs slightly with
    # P-dependent node weights.
    for k in range(gpu.num_elements * NP_p):
        hptr[k * NC + 0] = rho0
        hptr[k * NC + 1] = mx0
        hptr[k * NC + 2] = my0
