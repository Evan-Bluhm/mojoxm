# ======================================================================
# limiter_2d_gpu_test_p5 -- P=5 BJ limiter smoke test (NP=21)
# ======================================================================
#
# P=5 counterpart of limiter_2d_gpu_test_p4.  The mass-matrix-weighted
# cell mean differs at every P; this test brings limiter unit-test
# parity up to the largest 2D NP the bench harness exercises in
# shocked-flow gates (`bench_euler_sod_limited_2d_p5`).
#
# Same two checks as the P=2/P=3/P=4 tests:
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("limiter_2d_gpu_test_p5: runs at np=1 only")
        return
    print("limiter_2d_gpu_test_p5 (P=5 / NP=21 BJ limiter)")
    ...
    # node 0 of element 0 to rho=5.  Empirical max rho post-limit
    # at NP=21: ~1.045 (vs ~1.001 at NP=15 P=4 / ~2.0 at NP=6 P=2).
    # The post-limit value isn't monotone in P -- it depends on the
    # mass-matrix node weight at the perturbed node, which varies
    # non-trivially with P (e.g. tet vertex weights are NEGATIVE at
    # P>=2).  The 2.5 threshold remains a generous regression catch
    # for "did the limiter do anything" rather than a precise value.
    for k in range(gpu.num_elements * NP_p):
        hptr[k * NC + 0] = rho0
        hptr[k * NC + 1] = mx0
        hptr[k * NC + 2] = my0
