...
# by the lower-P tests.  This test closes the 3D BJ-limiter
# unit-test parity sweep at P=2/3/4/5.
# ======================================================================
...
def main() raises:
    ...
    if size > 1:
        mpi.finalize()
        print("limiter_3d_test_p5: runs at np=1 only")
        return

    print("limiter_3d_test_p5: P=5 / NP=56 BJ limiter conservation")
    ...
    if max_err_const > CONST_TOL:
        raise Error("limiter_3d_test_p5 FAILED: constant state altered by limiter by " + String(max_err_const))
    ...
    if max_mean_drift > MEAN_TOL:
        raise Error("limiter_3d_test_p5 FAILED: cell mean drifted by " + String(max_mean_drift) + " under BJ limiter (conservation broken)")
    ...
    if max_change < Float32(0.01):
        raise Error("limiter_3d_test_p5 FAILED: limiter appears not to have modified q (max_change=" + String(max_change) + "); test setup may be invalid")

    print("=== limiter_3d_test_p5 PASSED ===")
    mpi.finalize()
