...
def fill_constant_kernel(q: UnsafePointer[Float32, MutAnyOrigin], owned_elem_ids: UnsafePointer[Int32, MutAnyOrigin], num_owned: Int, value: Float32):
...
def main() raises:
    ...
    print("p3_smoke_test: building Mesh[", P, "] / Solver[Advection, ", P, "] at NP =", NP)
    ...
    print("  num_owned_elements:", num_owned, "  expected total_owned_dof:", num_owned * NP)
    print("  solver.total_owned_dof:", solver.total_owned_dof)
    if solver.total_owned_dof != num_owned * NP:
        raise Error("total_owned_dof mismatch: Solver reports " + String(solver.total_owned_dof) + " but num_owned_elements * NP = " + String(num_owned * NP))
