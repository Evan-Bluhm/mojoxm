...
def check_topology[P: Int](Nx: Int, Ny: Int, Lx: Float64, Ly: Float64) raises:
    ...
    for fid in range(mesh.num_faces):
        ...
        for m in range(NFP_e):
            ...
            if dxv > tol or dyv > tol:
                raise Error("face " + String(fid) + " slot " + String(m) + ": side-0 / side-1 node coords disagree:" + " dx=" + String(dxv) + " dy=" + String(dyv))
    ...
    print("    OK:", mesh.num_elements, "elements,", mesh.num_faces, "faces,", NP_p, "nodes/elem")
