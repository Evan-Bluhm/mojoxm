...
# ======================================================================
#
# Builds a tiny LocalMesh2D[2], synthesises three trivial scalar fields
...
def main() raises:
    print("vtu_2d_multi_test (multi-field VTU smoke test)")
    ...
    var path = String("/tmp/vtu_2d_multi_test.vtu")
    dump_vtu_2d_frame_multi[P](mesh, names, fields, path)

    var blob = Path(path).read_bytes()
    if len(blob) == 0:
        raise Error("vtu_2d_multi_test: output file is empty")
    ...
    if not (String('Name="p"') in s):
        raise Error("vtu_2d_multi_test: missing p DataArray header")
    if not (String('Name="|v|"') in s):
        raise Error("vtu_2d_multi_test: missing |v| DataArray header")
