...
def main() raises:
    ...
    print("mhd_3d_test: 3D IdealMHD constant-state preservation")
    ...
    var physics = IdealMHD(
        GAMMA, Float32(1.0e-6), Float32(1.0e-6), C_H, ALPHA_D,
    )
    ...
    solver.ctx.enqueue_function[
        fill_constant_kernel, fill_constant_kernel
    ](
    ...
    for c in range(NC):
        ...
        for k in range(n_dof):
            ...
            if isnan(v) or isinf(v):
                raise Error("mhd_3d_test: non-finite at component "
                            + String(c))
            var d = v - ic_vals[c]
    ...
    if max_err > CONST_TOL:
        raise Error(
            "mhd_3d_test FAILED: constant state shifted by "
