...
def check[P: Int](mut nvtx: NvtxContext) raises:
    ...
    for c in range(NC):
        ...
        for k in range(n_dof):
            ...
            if isnan(v) or isinf(v):
                raise Error("euler_3d_test P=" + String(P) + ": non-finite at component " + String(c))
            var d = v - ic_vals[c]
    ...
    if max_err > CONST_TOL:
        raise Error("euler_3d_test P=" + String(P) + " FAILED: constant state shifted by " + String(max_err) + " over " + String(NUM_STEPS) + " steps")
