...
def _upload_identity_i32(mut ctx: DeviceContext, n: Int) raises -> DeviceBuffer[patch_i]:
...
def offset_nodes_kernel(elem_node_xyz: UnsafePointer[Float32, MutAnyOrigin], num_points: Int, ox: Float32, oy: Float32, oz: Float32):
...
def gather_stride_i32_kernel(new_arr: UnsafePointer[Int32, MutAnyOrigin], old_arr: UnsafePointer[Int32, MutAnyOrigin], inv_perm: UnsafePointer[Int32, MutAnyOrigin], num_local: Int, stride: Int):
    var idx = Int(global_idx.x)
    var total = num_local * stride
    if idx >= total:
        return
...
struct Mesh[P: Int = 2](Movable):
    ...
    def __init__(out self, mut ctx: DeviceContext, var part: Partition, Lx: Float64, Ly: Float64, Lz: Float64, bcs: BoundaryConditions) raises:
        ...
        if part.px * part.py * part.pz == 1:
            self.local = LocalMesh[Self.P](ctx, part.nx, part.ny, part.nz, Lx, Ly, Lz, bcs)
            self.ghost_width = 0
        ...
        self.local = LocalMesh[Self.P](ctx, nx_loc, ny_loc, nz_loc, Lx_loc, Ly_loc, Lz_loc, rank_bcs, 1)
        self.ghost_width = 1
        ...
        ctx.enqueue_function[offset_nodes_kernel](self.local.d_elem_node_xyz.unsafe_ptr(), num_points, ox, oy, oz, grid_dim=ceildiv(num_points, PATCH_BLOCK), block_dim=PATCH_BLOCK)
        ...
        ctx.enqueue_function[build_owned_elem_ids_kernel,](
        ...
        ctx.enqueue_function[_gather_owned_initial](
        ...
        ctx.enqueue_function[classify_owned_kernel](
        ...
        _assign_ghost_ring(pp, ghost_cursor, 0, 0, -1, -1, 1, ny_p + 1, 1, nz_p + 1, 0, nx_loc, ny_loc)
        # +x ghost ring (lcx=nx+1).
        _assign_ghost_ring(pp, ghost_cursor, 1, nx_p + 1, -1, -1, 1, ny_p + 1, 1, nz_p + 1, 0, nx_loc, ny_loc)
        # -y ghost ring (lcy=0), iterate (lcx, lcz).
        _assign_ghost_ring(pp, ghost_cursor, 2, -1, 0, -1, 1, nx_p + 1, 1, nz_p + 1, 1, nx_loc, ny_loc)
        # +y ghost ring (lcy=ny+1).
        _assign_ghost_ring(pp, ghost_cursor, 3, -1, ny_p + 1, -1, 1, nx_p + 1, 1, nz_p + 1, 1, nx_loc, ny_loc)
        # -z ghost ring (lcz=0), iterate (lcx, lcy).
        _assign_ghost_ring(pp, ghost_cursor, 4, -1, -1, 0, 1, nx_p + 1, 1, ny_p + 1, 2, nx_loc, ny_loc)
        # +z ghost ring (lcz=nz+1).
        _assign_ghost_ring(pp, ghost_cursor, 5, -1, -1, nz_p + 1, 1, nx_p + 1, 1, ny_p + 1, 2, nx_loc, ny_loc)
        ...
        self.d_owned_elem_ids = _upload_identity_i32(ctx, self.num_owned_elements)
        ctx.synchronize()
        ...
        ctx.enqueue_function[_gather_owned_post](
    ...
    def _permute_mesh_arrays(mut self, mut ctx: DeviceContext, num_local: Int) raises:
        ...
        # elem_inv_6V).
        @parameter
        def _gather_f32(mut old: DeviceBuffer[patch_f], stride: Int) raises -> DeviceBuffer[patch_f]:
            var new = ctx.enqueue_create_buffer[patch_f](num_local * stride)
            var total = num_local * stride
            ctx.enqueue_function[gather_stride_f32_kernel](new.unsafe_ptr(), old.unsafe_ptr(), inv, num_local, stride, grid_dim=ceildiv(total, PATCH_BLOCK), block_dim=PATCH_BLOCK)
            return new^

        @parameter
        def _gather_i32(mut old: DeviceBuffer[patch_i], stride: Int) raises -> DeviceBuffer[patch_i]:
            var new = ctx.enqueue_create_buffer[patch_i](num_local * stride)
            var total = num_local * stride
            ctx.enqueue_function[gather_stride_i32_kernel](new.unsafe_ptr(), old.unsafe_ptr(), inv, num_local, stride, grid_dim=ceildiv(total, PATCH_BLOCK), block_dim=PATCH_BLOCK)
            return new^
        ...
        self.local.d_elem_canon_to_ref = _gather_i32(self.local.d_elem_canon_to_ref, N_F * 6)

        # Remap element-id VALUES inside face_elem (face entries
        # reference elements by id).
        var n_entries = self.local.num_faces * 2
        ctx.enqueue_function[remap_face_elem_values_kernel](self.local.d_face_elem.unsafe_ptr(), perm, n_entries, grid_dim=ceildiv(n_entries, PATCH_BLOCK), block_dim=PATCH_BLOCK)
        ctx.synchronize()
