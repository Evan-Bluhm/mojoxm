...
def mhd_glm_vol_lift_combine_rk_kernel_2d[
    NP: Int, NFP: Int
](
...
def launch_mhd_glm_vol_lift_2d[
    NP: Int, NFP: Int
](
...
def mhd_glm_face_flux_kernel_2d[
    NP: Int, NFP: Int
](
    ...
    fstar_out[out + 6] = half * ((FxL6 + FxR6) * nx + (FyL6 + FyR6) * ny) - half * alpha * (qR6 - qL6)


def launch_mhd_glm_face_flux_2d[
    NP: Int, NFP: Int
](
    mut ctx: DeviceContext,
    q: UnsafePointer[Float32, MutAnyOrigin],
...
def mhd_glm_psi_damp_kernel_2d[NP: Int](q: UnsafePointer[Float32, MutAnyOrigin], num_elements: Int, decay: Float32):
    var tid = Int(global_idx.x)
    var total = num_elements * NP
    if tid >= total:
        return
...
def launch_mhd_glm_psi_damp_2d[NP: Int](mut ctx: DeviceContext, q: UnsafePointer[Float32, MutAnyOrigin], num_elements: Int, alpha_d: Float32, dt: Float32) raises:
    ...
    ctx.enqueue_function[_kernel](q, num_elements, decay, grid_dim=ceildiv(total, 256), block_dim=256)


def mhd_glm_rk_stage_2d[
    P: Int
](
    mut ctx: DeviceContext,
