...
comptime FRAME_PREFIX = "frame_sod2d_gpu_"
...
def main() raises:
    ...
    var d_cell_avg_count = gpu_mesh.num_elements * (NC + 1)
    var d_cell_avg = ctx.enqueue_create_buffer[DType.float32](d_cell_avg_count)

    # WARPXM-style device-memory accounting (Euler Sod with HLLC + BJ
    # limiter; 2D np=1 only).
    MemoryReport(
        rk_stage_bytes=3 * n_q * 4,
        dg_operators_bytes=gpu_re.device_bytes(),
        limiter_bytes=d_cell_avg_count * 4,
