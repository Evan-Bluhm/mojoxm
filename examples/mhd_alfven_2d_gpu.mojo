...
from src.local_mesh_2d_gpu import LocalMesh2DGpu, mhd_rk_stage_2d
...
def _frame_name(i: Int) raises -> String:
    var s = String("frame_mhd_alfven_2d_gpu_")
    var idx = String(i)
...
def main() raises:
    ...
    dump_vtu_2d_frame[P](mesh_coords, By_scalar, f0_path, String("By"))
    ...
    for fi in range(1, NUM_FRAMES + 1):
        ...
        dump_vtu_2d_frame[P](mesh_coords, By_scalar, path_i, String("By"))
    ...
    pvd += '</Collection>\n'
    pvd += '</VTKFile>\n'
    Path("output/solution_mhd_alfven_2d_gpu.pvd").write_text(pvd)
